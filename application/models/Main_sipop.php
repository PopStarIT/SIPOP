...
class Main_sipop extends CI_Model
{
...
from dbo.dt_mst_item_base a
  ...
  where	i.custom_item_type_id = 1 and a.item_base_code='" . $item_name . "'";
	...
	function cari_fabric_receive_header($fabric_warehouse_receive_id){
		$sql = "select a.fabric_warehouse_receive_status_id, b.grn_status, b.lock_data, a.fabric_warehouse_receive_date, a.fabric_warehouse_receive_no, a.fabric_warehouse_receive_type_id
	, a.partner_id,a.fabric_shipment_id,a.warehouse_id, a.bc_in_header_id, a.no_surat_jalan,
	a.tgl_surat_jalan, a.purchase_order_id, a.vendor_partner_id 
	     from dbo.dt_fabric_warehouse_receive a
		Left Join dbo.prm_grn_status b ON a.fabric_warehouse_receive_status_id = b.grn_status_id
  ...
  GROUP BY c.bom_id, c.bom_code, b.item_id, b.item_code, b.item_name, d.uom_code, f.quantity_plan, f.quantity_reject,g.supply";
	...
	function error_bahan_baku($date_start, $date_end)
	{
		//$date_start='2025-09-09';
		//$date_end='2025-10-09';

		$sql = "select count(a.item_base_code) as minus_bahan_baku
		 from dbo.dt_mst_item_base a
	     Left Join dbo.ref_uom b ON a.uom_id = b.uom_id
	     Left Join dbo.sp_mutasi_saldo_awal('" . $date_start . "') c ON a.item_base_id = c.item_base_id
	     Left Join dbo.sp_mutasi_pemasukan('" . $date_start . "','" . $date_end . "') d ON a.item_base_id = d.item_base_id
	     ...
	     LEFT JOIN dbo.dt_mst_item_category h ON a.item_category_id = h.item_category_id
	     Left Join dbo.prm_custom_item_type i ON h.custom_item_type_id = i.custom_item_type_id
         where	i.custom_item_type_id = 1 AND (COALESCE(c.quantity,0) + COALESCE(d.quantity,0) + COALESCE(e.quantity,0) + COALESCE(f.quantity,0)) < 0 ";
		$query = $this->db->query($sql);
		return $query->result_array();
		//$this->db_pop->select('count(work_order_request_id)');
		//$this->db_pop->from('dbo.view_check_prod_request_dan_transfer');
	...
	function jml_dok_bc_supply_kosong($date_start, $date_end)
	{
		$sql = "SELECT count(a.bc_out_header_status_id) FROM dbo.dt_bc_out_header a
         LEFT JOIN dbo.dt_bc_out_barang b ON a.bc_out_header_id=b.bc_out_header_id
         LEFT JOIN dbo.dt_mst_item c ON b.item_id=c.item_id
         LEFT JOIN dbo.dt_bc_out_barang_supply d ON b.bc_out_barang_id=d.bc_out_barang_id
         LEFT JOIN dbo.prm_custom_type e ON a.custom_type_id=e.custom_type_id
		 WHERE d.stock_move_id is null and a.bc_out_header_status_id=1 and d.quantity_supply > 0 and a.bc_date >= '" . $date_start . "' AND a.bc_date <= '" . $date_end . "'";
		$query = $this->db->query($sql);
		return $query->result_array();
		//  WHERE d.stock_move_id is null and a.bc_out_header_status_id=1 and d.quantity_supply > 0 and a.bc_date >= '2025-05-03' AND a.bc_date <= '2025-08-22'";
	}

	function jml_tanggal_prod_transfer_lebih($date_start)
	{
		$sql = "Select count(a.work_order_transfer_id) From dbo.dt_work_order_transfer a
	    LEFT JOIN dbo.dt_work_order_transfer_detail b ON a.work_order_transfer_id = b.work_order_transfer_id
	    LEFT JOIN dbo.dt_mst_item c ON b.item_id=c.item_id
	    Left Join dbo.dt_stock_move d ON b.stock_move_id=d.stock_move_id
	    left join dbo.dt_work_order_request_list e ON b.work_order_request_list_id=e.work_order_request_list_id
	     where a.work_order_transfer_date < d.stock_move_date and a.work_order_transfer_date >'" . $date_start . "'";
		$query = $this->db->query($sql);
		return $query->result_array();
		//  WHERE d.stock_move_id is null and a.bc_out_header_status_id=1 and d.quantity_supply > 0 and a.bc_date >= '2025-05-03' AND a.bc_date <= '2025-08-22'";
	}

	function view_tanggal_prod_transfer_lebih($date_start)
	{
		$sql = "Select 
		a.work_order_transfer_id as r1,
		'Prod Transfer'::text AS r2,   
		a.work_order_transfer_no as r3,
		a.work_order_transfer_date as r4,
		d.stock_move_date as r5,
		c.item_code as r6,
		e.quantity_request as r7,
		b.quantity_supply as r8 
		From dbo.dt_work_order_transfer a
	    LEFT JOIN dbo.dt_work_order_transfer_detail b ON a.work_order_transfer_id = b.work_order_transfer_id
	    LEFT JOIN dbo.dt_mst_item c ON b.item_id=c.item_id
	    Left Join dbo.dt_stock_move d ON b.stock_move_id=d.stock_move_id
	    left join dbo.dt_work_order_request_list e ON b.work_order_request_list_id=e.work_order_request_list_id
	...
	function jml_beda_kode_barang_supply_transfer($date_start)
	{
		$sql = "select count(work_order_transfer_no)
		FROM dbo.dt_work_order_transfer a 
		left join dbo.dt_work_order_transfer_detail b ON a.work_order_transfer_id=b.work_order_transfer_id
		left join dbo.dt_mst_item c ON b.item_id=c.item_id
		LEFT JOIN dbo.dt_stock_move d ON b.stock_move_id=d.stock_move_id 
		left join dbo.dt_mst_item e ON d.item_id=e.item_id
		left join dbo.dt_grn_detail f ON d.grn_detail_id=f.grn_detail_id
		LEFT JOIN dbo.dt_grn g ON f.grn_id=g.grn_id
		LEFT JOIN dbo.dt_bc_in_header h ON g.bc_in_header_id=h.bc_in_header_id
		where c.item_code <> e.item_code and a.work_order_transfer_date >='" . $date_start . "'";
		$query = $this->db->query($sql);
		return $query->result_array();
		//'2025-09-11'
	}

	function view_beda_kode_barang_supply_transfer($date_start)
	{
		$sql = "select d.stock_move_id as r1,
		'Prod.transfer'::text as r2
		,work_order_transfer_no as r3
		,work_order_transfer_date as r4 
	    ,c.item_code as r5
		,e.item_code as r6
		,b.quantity_supply as r7
		,g.grn_no as r8
		,g.grn_date as r9
		FROM dbo.dt_work_order_transfer a 
		left join dbo.dt_work_order_transfer_detail b ON a.work_order_transfer_id=b.work_order_transfer_id
	...
	public function view_asset_incoming_detail($id) {}

	public function get_task()
	{
		$this->db_pop->select('*');
		$this->db_pop->from('dbo.view_style_no_task_assignment_rft');
		$query = $this->db_pop->get();
		return $query->result_array(); // Return as an array
	}
	
	public function get_id_by_warna_remote($warna_target) {
          $this->load->driver('cache', array('adapter' => 'file'));
	        
           // 1. Cek apakah daftar warna dari remote sudah ada di cache lokal
              $list_warna_lain = $this->cache->get('master_warna_remote');
	        
              if (!$list_warna_lain) {
                  // Jika cache kosong, tarik data via DBLink
                  // Ganti host, user, password, dan dbname sesuai server remote Anda
                  $conn_str = "host=192.168.1.100 user=postgres password=rahasia dbname=inventory_db";
                  
                  $sql_remote = "
                      SELECT DISTINCT warna FROM dblink('$conn_str', 
                      'SELECT warna FROM item_barang WHERE warna IS NOT NULL') 
                      AS t(warna text)
                  ";
	        
                  $query_remote = $this->db->query($sql_remote)->result_array();
	        
                  $temp_warna = [];
                  foreach ($query_remote as $row) {
                      // Pecah simbol (/, -, #) menjadi spasi agar kata berdiri sendiri
                  ...
                  // Simpan ke cache lokal selama 1 jam (3600 detik)
                  $this->cache->save('master_warna_remote', $list_warna_lain, 3600);
              }
	        
              // 2. Filter daftar warna (buang warna_target dari daftar warna lain)
              $warna_target_lower = strtolower($warna_target);
              $warna_pembanding = array_filter($list_warna_lain, function($v) use ($warna_target_lower) {
                  return $v !== $warna_target_lower;
              });
	        
              // 3. Query Utama untuk mencari ID ke Server Remote via DBLink
              // Kita menggunakan Regex PostgreSQL (~*) untuk validasi murni (tanpa simbol)
              $regex_murni = "(^|[[:space:]])" . preg_quote($warna_target) . "([[:space:]]|$)";
              $regex_lain = !empty($warna_pembanding) ? implode('|', array_map('preg_quote', $warna_pembanding)) : 'NONE';
	        
              $conn_str = "host=192.168.1.100 user=postgres password=rahasia dbname=inventory_db";
              $final_sql = "
                  SELECT * FROM dblink('$conn_str', 
                  'SELECT id, warna FROM item_barang 
                   WHERE warna ~* ''$regex_murni'' 
                   AND warna NOT ~* ''($regex_lain)''') 
                  AS t(id int, warna text)
              ";
