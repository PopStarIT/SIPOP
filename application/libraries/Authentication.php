...
Class Authentication  {
	...
	//	print_r($user_id);
		//print_r($role_config[$role_id]);
		//var_dump($role_config[$role_id]);
		//var_dump($role_username_config);
		//var_dump($role_username_config);
		//var_dump($role_config[$role_id]);
		$securityid = 0;
		if ($role_id == '-99'){
			...
			if(array_key_exists($role_id,$role_config)){
			 // var_dump(array_key_exists($user_id,$role_username_config));die();
			  // var_dump(in_array($user_id,$role_username_config));die();
				if (array_key_exists($user_id,$role_username_config)){
			...
			if(!$maintenance_mode) {
				...
				if($methodid != 0){		
					...
					if($tokenid == -'3'){
						...
						//var_dump($this->CI->session->userdata('login'));die();
						//print_r($_SERVER['HTTP_X_REQUESTED_WITH']);
						if(!$this->CI->session->userdata('login')){
							//var_dump(isset($_SERVER['HTTP_X_REQUESTED_WITH']));die();
							if(isset($_SERVER['HTTP_X_REQUESTED_WITH'])){
							...
							//var_dump($this->CI->session->userdata('lockscreen'));die();
							if($this->CI->session->userdata('lockscreen')){
								$component = array();
								$this->lock($component);
							} 
					...
					//var_dump($this->CI->session->userdata('role_id'));die();
					if($this->CI->session->userdata('role_id') != -99){
						//var_dump($this->CI->session->userdata('role_id'));
					//print_r($_SERVER);
  ...
  function layout_dashboard_preview($array=array()){
		$data['menu'] = '';
		$data['class_uri'] = $this->generate_class_uri();
	...
	function permission_check($method_id=0){
		$permission = false;
		$role_id  = $this->CI->session->userdata('role_id'); //54
	//	echo $method_id;
		//var_dump($data_method[25]);
		if($method_id != 0){
			$data_method = $this->data['data_method'];
			//var_dump($this->role_config[54]);die();
			if(isset($data_method[$method_id])){
				$dt_menu = $data_method[$method_id];
				$token_id = $dt_menu['token_id']; //44
				//var_dump($dt_menu);die();
				if($token_id == -2){
					if(!$this->CI->session->userdata('login')){
						$permission = true;
					}
