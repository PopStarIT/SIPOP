...
class Rpc_service
{
	...
	public function setSP($params)
	{
		...
		$this->XML->loadXML($XMLDATA_info, LIBXML_NOENT | LIBXML_XINCLUDE | LIBXML_NOERROR | LIBXML_NOWARNING);
		// var_dump();die();
		//  var_dump( $this->XML->loadXML($XMLDATA_info, LIBXML_NOENT | LIBXML_XINCLUDE | LIBXML_NOERROR | LIBXML_NOWARNING));die();
		// var_dump();die();
		//echo $this->XML->saveXML(); 
		unset($ua);
		unset($session_data);
	...
	public function resultJSON($prepare = 'my_query', $withLog = false, $jsonformat = false, $die = false)
	{
		...
		try {
			...
			$dbconn = pg_connect("host=192.168.99.133 port=" . PG_PORT2 . " dbname=" . PG_DB2 . " user=" . PG_USER2 . " password=" . PG_PASS2);
			if (!$dbconn) {
	...
	function searchAddGroup($modeGroup)
	{	//$modeGroup 0=AND ,1 =OR
		$node = $this->XML->getElementsByTagName('param')->item(0);
	...
	//$this->searchAddText($this->group,r5,0,1,'PS/1157-24/TEAL');  
	function searchAddText(
		$groupSearch	// target group 
		,
	...
	public function print_specification($prepare = 'my_query', $withLog = false, $jsonformat = false, $die = false, $format, $header_id, $print, $xs = false, $xl = false, $xxl = false)
	{
		$sts;
		$stshdr;
		$des;
		...
		try {
			...
			$id_header = $data_json->xrow_header[0]->style_spec_header_id;
			$id_basic = $this->CI->main->find_data_pop('dbo.dt_style_spec_detail', 'style_spec_header_id', $id_header, 1);
			//var_dump(count($id_basic));die();

			$cek_basic = $this->CI->main->find_data_pop('dbo.dt_pilih', 'pilih_id', $id_basic[0]['style_spec_detail_size_id'], 1);

			$basic = $cek_basic[0]['uraian'];
			//var_dump($cek_basic[0]['uraian']);die();
			...
			if ($format == 'pdf') {
				//	var_dump("PDF Aktif basic: ".$basic." (semuanya)");die();
				if ($xs == 'true') {
					if ($xl == 'true') {
						if ($xxl == 'true') {
							switch ($basic) {
								case 'S':
									$template = 'C:/tmp_sipop/specification/pdf/all/specification_all_s.fodt';
									break;
								case 'M':
									// var_dump('basic M');
									$template = 'C:/tmp_sipop/specification/pdf/all/specification_all_m.fodt';
									break;
								default:
									// var_dump('basic L');
									$template = 'C:/tmp_sipop/specification/pdf/all/specification_all_l.fodt';
									break;
							}
							$template_detail_data = 'C:/tmp_sipop/specification/pdf/all/specification_all_detail.fodt';
							$template_trims_data = 'C:/tmp_sipop/specification/pdf/all/specification_all_trims.fodt';
							$template_history_data = 'C:/tmp_sipop/specification/pdf/all/specification_all_history.fodt';
						} else {
							//var_dump('xs dan xl aktif');die();
							//echo "<script type='text/javascript'>alert('Oop..sory! Untuk PDF pilihan xs dan xl aktif masih proses pengembangan !');</script>";
							//var_dump('xs dan xl aktif');die();
						}
					} else {
						if ($xxl == 'true') {
							//var_dump('xs dan xxl aktif');
						} else {
							//var_dump('xs aktif');
						}
					}
					...
					if ($xl == 'true') {
						...
						if ($xxl == 'true') {
							...
							//var_dump('semuanya tidak aktif');
							switch ($basic) {
								case 'S':
									$template = 'C:/tmp_sipop/specification/pdf/default/specification_default_s.fodt';
									break;
								case 'M':
									// var_dump('basic M');
									$template = 'C:/tmp_sipop/specification/pdf/default/specification_default_m.fodt';
									break;
								default:
									// var_dump('basic L');
									$template = 'C:/tmp_sipop/specification/pdf/default/specification_default_l.fodt';
									break;
							}
							$template_detail_data = 'C:/tmp_sipop/specification/pdf/default/specification_default_detail.fodt';
							$template_trims_data = 'C:/tmp_sipop/specification/pdf/default/specification_default_trims.fodt';
							$template_history_data = 'C:/tmp_sipop/specification/pdf/default/specification_default_history.fodt';
						}
					}
				}
				...
				// jika menggunakan Extension xlsx untuk image tidak bisa tampil 
				if ($xs == 'true') {
					if ($xl == 'true') {
						if ($xxl == 'true') {
							//var_dump("XS dan XL dan XXL Aktif basic: ".$basic." (semuanya)");die();
							switch ($basic) {
								case 'S':
									$template = 'C:/tmp_sipop/specification/excel/detail/all/specification_all_s.fods';
									break;
								case 'M':
									$template = 'C:/tmp_sipop/specification/excel/detail/all/specification_all_m.fods';
									break;
								default:
									$template = 'C:/tmp_sipop/specification/excel/detail/all/specification_all_l.fods';
									break;
							}
							$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/all/specification_all_detail.fods';
							$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/all/specification_all_trims.fods';
							$template_history_data = 'C:/tmp_sipop/specification/excel/detail/all/specification_all_history.fods';
						} else {
							//var_dump("XS dan XL Aktif basic: ".$basic);
							switch ($basic) {
								case 'S':
									$template = 'C:/tmp_sipop/specification/excel/detail/xxl_tidak_aktif/specification_non_xxl_s.fods';
									break;
								case 'M':
									$template = 'C:/tmp_sipop/specification/excel/detail/xxl_tidak_aktif/specification_non_xxl_m.fods';
									break;
								default:
									$template = 'C:/tmp_sipop/specification/excel/detail/xxl_tidak_aktif/specification_non_xxl_l.fods';
									break;
							}
							$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/xxl_tidak_aktif/specification_non_xxl_detail.fods';
							$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/xxl_tidak_aktif/specification_non_xxl_trims.fods';
							$template_history_data = 'C:/tmp_sipop/specification/excel/detail/xxl_tidak_aktif/specification_non_xxl_history.fods';
						}
					} else if ($xxl == 'true') {
						//var_dump("XS dan XXL Aktif basic: ".$basic);
						switch ($basic) {
							case 'S':
								$template = 'C:/tmp_sipop/specification/excel/detail/xs dan xxl_aktif/specification_xs_xxl_s.fods';
								break;
							case 'M':
								$template = 'C:/tmp_sipop/specification/excel/detail/xs dan xxl_aktif/specification_xs_xxl_m.fods';
								break;
							default:
								$template = 'C:/tmp_sipop/specification/excel/detail/xs dan xxl_aktif/specification_xs_xxl_l.fods';
								break;
						}
						$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/xs dan xxl_aktif/specification_xs_xxl_detail.fods';
						$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/xs dan xxl_aktif/specification_xs_xxl_trims.fods';
						$template_history_data = 'C:/tmp_sipop/specification/excel/detail/xs dan xxl_aktif/specification_xs_xxl_history.fods';
					} else {
						// var_dump("XS aktif basic: ".$basic);
						switch ($basic) {
							case 'S':
								$template = 'C:/tmp_sipop/specification/excel/detail/xs_aktif/specification_xs_s.fods';
								break;
							case 'M':
								$template = 'C:/tmp_sipop/specification/excel/detail/xs_aktif/specification_xs_m.fods';
								break;
							default:
								$template = 'C:/tmp_sipop/specification/excel/detail/xs_aktif/specification_xs_l.fods';
								break;
						}
						$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/xs_aktif/specification_xs_detail.fods';
						$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/xs_aktif/specification_xs_trims.fods';
						$template_history_data = 'C:/tmp_sipop/specification/excel/detail/xs_aktif/specification_xs_history.fods';
					}
				} else if ($xl == 'true') {
					if ($xxl == 'true') {
						// var_dump("Xl dan XXL Aktif basic: ".$basic);
						switch ($basic) {
							case 'S':
								$template = 'C:/tmp_sipop/specification/excel/detail/xl_xxl_aktif/specification_xl_xxl_s.fods';
								break;
							case 'M':
								$template = 'C:/tmp_sipop/specification/excel/detail/xl_xxl_aktif/specification_xl_xxl_m.fods';
								break;
							default:
								$template = 'C:/tmp_sipop/specification/excel/detail/xl_xxl_aktif/specification_xl_xxl_l.fods';
								break;
						}
						$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/xl_xxl_aktif/specification_xl_xxl_detail.fods';
						$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/xl_xxl_aktif/specification_xl_xxl_trims.fods';
						$template_history_data = 'C:/tmp_sipop/specification/excel/detail/xl_xxl_aktif/specification_xl_xxl_history.fods';
					} else {
						//var_dump("XL Aktif basic: ".$basic);
						switch ($basic) {
							case 'S':
								$template = 'C:/tmp_sipop/specification/excel/detail/xl_aktif/specification_xl_s.fods';
								break;
							case 'M':
								$template = 'C:/tmp_sipop/specification/excel/detail/xl_aktif/specification_xl_m.fods';
								break;
							default:
								$template = 'C:/tmp_sipop/specification/excel/detail/xl_aktif/specification_xl_l.fods';
								break;
						}
						$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/xl_aktif/specification_xl_detail.fods';
						$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/xl_aktif/specification_xl_trims.fods';
						$template_history_data = 'C:/tmp_sipop/specification/excel/detail/xl_aktif/specification_xl_history.fods';
					}
				} else {
					if ($xxl == 'true') {
						//var_dump("XXL Aktif basic: ".$basic);
						switch ($basic) {
							case 'S':
								$template = 'C:/tmp_sipop/specification/excel/detail/xxl_aktif/specification_xxl_s.fods';
								break;
							case 'M':
								$template = 'C:/tmp_sipop/specification/excel/detail/xxl_aktif/specification_xxl_m.fods';
								break;
							default:
								$template = 'C:/tmp_sipop/specification/excel/detail/xxl_aktif/specification_xxl_l.fods';
								break;
						}
						$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/xxl_aktif/specification_xxl_detail.fods';
						$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/xxl_aktif/specification_xxl_trims.fods';
						$template_history_data = 'C:/tmp_sipop/specification/excel/detail/xxl_aktif/specification_xxl_history.fods';
					} else {
						switch ($basic) {
							case 'S':
								//var_dump("XS dan XL dan XXL tidak aktif (Default) basic: S ");
								$template = 'C:/tmp_sipop/specification/excel/detail/default/specification - default_s.fods';
								break;
							case 'M':
								//var_dump("XS dan XL dan XXL tidak aktif (Default) basic: M ");
								$template = 'C:/tmp_sipop/specification/excel/detail/default/specification - default_m.fods';
								break;
							default:
								//var_dump("XS dan XL dan XXL tidak aktif (Default) basic: L ");
								$template = 'C:/tmp_sipop/specification/excel/detail/default/specification - default_l.fods';
								break;
						}
						$template_detail_data = 'C:/tmp_sipop/specification/excel/detail/default/specification_default_detail.fods';
						$template_trims_data = 'C:/tmp_sipop/specification/excel/detail/default/specification_default_trims.fods';
						$template_history_data = 'C:/tmp_sipop/specification/excel/detail/default/specification_default_history.fods';
						$template_image_data_old = 'C:/tmp_sipop/specification/excel/lap_spec_image_data.fods';
						// $template_image_data='C:/tmp_sipop/specification/excel/draw_image.fods';
				...
				//	if ($basic=='M'){
				//	  $template='C:/tmp_sipop/specification/excel/lap_specification-m.fods';	
				//	}else if ($basic=='S'){
				//	  $template='C:/tmp_sipop/specification/excel/lap_specification-s.fods';
				//	}else if ($basic=='L'){
				//	  $template='C:/tmp_sipop/specification/excel/lap_specification-l.fods';
				//	}else{
				//	  $template='C:/tmp_sipop/specification/excel/lap_specification.fods';
				//	}
				//	$template_detail_data='C:/tmp_sipop/specification/excel/lap_spec_detail_data.fods';
				//	$template_trims_data='C:/tmp_sipop/specification/excel/lap_spec_trims_data.fods';
				//	$template_history_data='C:/tmp_sipop/specification/excel/lap_spec_history_data.fods';
				//	$template_image_data_old='C:/tmp_sipop/specification/excel/lap_spec_image_data.fods';
				//	$template_image_data='C:/tmp_sipop/specification/excel/draw_image.fods';
				$tmp_ext = 'fods';
				$EXTENSION = 'xls';
			...
			// nama file output / hasil
			$report_name = 'SPECIFICATION';

			// ambil isi file template 

			$template_doc = file_get_contents($template);
			$template_detail = file_get_contents($template_detail_data);
			$template_trims = file_get_contents($template_trims_data);
			$template_history = file_get_contents($template_history_data);
			// $template_image=file_get_contents($template_image_data);
			...
			//$data_json->xrow_header[0]->style_spec_nomor
			$isi_header = array(
				$data_json->xrow_header[0]->style_spec_nomor,
				$data_json->xrow_header[0]->item_code,
				$data_json->xrow_header[0]->style_spec_buyer,
				$data_json->xrow_header[0]->style_spec_date,
				$data_json->xrow_header[0]->style_spec_po,
				$data_json->xrow_header[0]->style_spec_pattern,
				$data_json->xrow_header[0]->style_spec_note,
				$data_json->xrow_header[0]->style_spec_fabric,
				$data_json->xrow_header[0]->style_spec_susut,
				$data_json->xrow_header[0]->style_spec_status
			);

			$dt_header = '';
			foreach ($isi_header as $isi) {
				$dt_header .= $isi;
			};

			$value_header = str_replace($doc_header, $isi_header, $template_doc);
			$header_id = $data_json->xrow_header[0]->style_spec_header_id;
			$hsl_detail = $this->CI->main->spec_detail($header_id, $basic);
			$hsl_trims = $this->CI->main->find_table_pop('dbo.dt_style_spec_trims', 'style_spec_header_id', $header_id);
			$hsl_history = $this->CI->main->find_table_pop('dbo.dt_style_spec_history', 'style_spec_header_id', $header_id);
			$hsl_img_1 = $this->CI->main->cari_gambar('style_spec_header_id', $header_id, 'style_spec_images_seri', 'pic_1', 1);
			$hsl_img_2 = $this->CI->main->cari_gambar('style_spec_header_id', $header_id, 'style_spec_images_seri', 'pic_2', 1);

			//var_dump($hsl_trims);die();
			$DOC = '';
			$value_doc = '';
			$value_doc2 = '';
			...
			$detail = $data_json->xrow_detail;
			$no_image = number_format(169.48);
			$img_pdf = 0.1628;
			//$y='15,4';

			$x = 0;
			foreach ($hsl_detail as $content) {
				//foreach($hsl_detail as $element=>$content){
				if ($content['r5'] == '0.000000') {
					$content['r6'] = '';
					$content['r7'] = '';
					$content['r8'] = '';
					$content['r9'] = '';
				...
				$xs = $this->CI->main->decimalToFraction2($content['r6']);
				$pat_xs = $this->CI->main->decimalToFraction2($content['r7']);
				$all_xs = $this->CI->main->decimalToFraction2($content['r8']);

				$s = $this->CI->main->decimalToFraction2($content['r9']);
				$pat_s = $this->CI->main->decimalToFraction2($content['r10']);
				$all_s = $this->CI->main->decimalToFraction2($content['r11']);

				$m = $this->CI->main->decimalToFraction2($content['r12']);
				$pat_m = $this->CI->main->decimalToFraction2($content['r13']);
				$all_m = $this->CI->main->decimalToFraction2($content['r14']);

				$l = $this->CI->main->decimalToFraction2($content['r15']);
				$pat_l = $this->CI->main->decimalToFraction2($content['r16']);
				$all_l = $this->CI->main->decimalToFraction2($content['r17']);

				$xl = $this->CI->main->decimalToFraction2($content['r18']);
				$pat_xl = $this->CI->main->decimalToFraction2($content['r19']);
				$all_xl = $this->CI->main->decimalToFraction2($content['r20']);

				$xxl = $this->CI->main->decimalToFraction2($content['r21']);
				$pat_xxl = $this->CI->main->decimalToFraction2($content['r22']);
				$all_xxl = $this->CI->main->decimalToFraction2($content['r23']);

				$value_doc .= str_replace($var_detail, array(
					$y[$x],
					$content['r4'],
					$xs,
					$pat_xs,
				...
				$x = $x + 1;
				$no_image = $no_image + number_format(19.4);
				$img_pdf = $img_pdf + 0.0020;
			};

			$tr = 1;
			$value_trims = '';
			$var_trims = array('{no_trims}', '{trims}');
			foreach ($hsl_trims as $con_trims) {

				$trims_description = $con_trims['style_spec_trims_description'];
				$value_trims .= str_replace($var_trims, array($tr, $trims_description), $template_trims);
				$tr = $tr + 1;
				$no_image = $no_image + number_format(16.2);
				$img_pdf = $img_pdf + 0.0020;
			};
			...
			$htr = 1;
			$value_history = '';
			$var_history = array('{no_history}', '{uraian_history}', '{value_history}', '{edit_value}');
			foreach ($hsl_history as $con_history) {

				$history_uraian = $con_history['spec_history_detail_uraian'];
				$history_value = $con_history['spec_history_detail_value'];
				$history_edit = $con_history['spec_history_detail_edit'];
				$value_history .= str_replace($var_history, array($htr, $history_uraian, $history_value, $history_edit), $template_history);
				$htr = $htr + 1;
				$no_image = $no_image + number_format(16.2);
				$img_pdf = $img_pdf + 0.0020;
			};
			//var_dump($y);die();
			//var_dump($value_history);die();	
			//$img_nama='';
			$img_1 = 1;
			$value_img_1 = '';
			$var_img_1 = '{gambar}';

			//--untuk membuat folder di folder temp
			//$folder_tmp='C:/tmp_img/';
			//    $folder_tmp=$temp.'/tmp_img/';
			//	//var_dump($folder_tmp);die();
			//   // $dir=false;
			//    if(!is_dir($folder_tmp)){
			//      $dir=mkdir($folder_tmp,0777,true);
			//	  $dir=mkdir($folder_tmp);
			//   }else{
			//      $dir=true;  
			//   }
			//APPPATH.
			foreach ($hsl_img_1 as $con_img) {
				$pecah = explode('/', $con_img['style_spec_name_images']);
				//   var_dump($img_pdf.'in');die();	
				//echo base_url('assets/img/pattern/contoh.png')
				$nm_file = $con_img['style_spec_name_images'];
				$lok_1 = base_url('assets/img/pattern/' . $con_img['style_spec_name_images']);
				//$lok_1='../assets/img/pattern/'.$con_img['style_spec_name_images'];
				$lok = 'file:///D:/xampp/htdocs/SIPOP/assets/img/pattern/' . $con_img['style_spec_name_images'];
				$lok2 = 'D:/xampp/htdocs/SIPOP/assets/img/pattern/' . $con_img['style_spec_name_images'];
				$lok3 = 'D:/xampp/htdocs/SIPOP/assets/img/pattern/PTR-2024_05_25-0005/041315_Capture-1.png';

				if ($format == 'pdf') {
					$ukuran = $img_pdf . 'in';
					$img_nama = "<draw:frame draw:style-name='fr1' draw:name='Image1' text:anchor-type='paragraph' svg:x='0.1264in' svg:y='" . $ukuran . "' svg:width='4.4291in' svg:height='3.0028in' draw:z-index='1'><draw:image xlink:href='" . $lok_1 . "' xlink:type='simple' xlink:show='embed' xlink:actuate='onLoad'/></draw:frame>";
					//	$value_img_1 .=str_replace($var_img_1,$lok,$template_image);
					$img_1 = $img_1 + 1;
				} else {
					$ukuran = $no_image . 'pt';
					$img_nama = "<draw:frame draw:z-index='1' draw:name='Image 2' draw:style-name='gr2' draw:text-style-name='P2' svg:width='450pt' svg:height='261.58pt' svg:x='40.53pt' svg:y='" . $ukuran . "'><draw:image xlink:href='" . $lok_1 . "'><text:p/></draw:image></draw:frame>";
					//	$value_img_1 .=str_replace($var_img_1,$lok,$template_image);
					$img_1 = $img_1 + 1;
				}
			};
			if (count($hsl_img_1) == '0') {
				$img_nama = "";
			}
			...
			foreach ($hsl_img_2 as $con_img2) {
				//$pecah2=explode('/', $con_img2['style_spec_name_images']);
				$lok_2 = base_url('assets/img/pattern/' . $con_img2['style_spec_name_images']);


				if ($format == 'pdf') {
					$ukuran = $img_pdf . 'in';
					$img_nama2 = "<draw:frame draw:style-name='fr1' draw:name='Image2' text:anchor-type='paragraph' svg:x='5.7929in' svg:y='" . $ukuran . "' svg:width='4.4291in' svg:height='3.0028in' draw:z-index='2'><draw:image xlink:href='" . $lok_2 . "' xlink:type='simple' xlink:show='embed' xlink:actuate='onLoad' loext:mime-type='image/png'/></draw:frame>";
				} else {
					$ukuran = $no_image . 'pt';
					$img_nama2 = "<draw:frame draw:z-index='1' draw:name='Image 2' draw:style-name='gr2' draw:text-style-name='P2' svg:width='450pt' svg:height='261.58pt' svg:x='540.53pt' svg:y='" . $ukuran . "'><draw:image xlink:href='" . $lok_2 . "'><text:p/></draw:image></draw:frame>";
				}
			};

			if (count($hsl_img_2) == '0') {
				$img_nama2 = "";
			}
			// closedir($dh);
			// var_dump($img_nama);die();	
			$isi_header2 = $data_json->xrow_header[0]->style_spec_nomor;
			$dataheader = array(
				$data_json->xrow_header[0]->style_spec_nomor,
				$data_json->xrow_header[0]->item_code,
				$data_json->xrow_header[0]->style_spec_buyer,
			...
			$DOC = str_replace($doc_header, $dataheader, $template_doc);
			//var_dump($DOC);die();	
			file_put_contents($temp . $report_name . $report_time . '.' . $tmp_ext, $DOC);
			// Gunakan perintah unoconv untuk mengkonversi file .fods ke Excel
	...
	public function print_specification_backup($prepare = 'my_query', $withLog = false, $jsonformat = false, $die = false, $format, $header_id, $print)
	{
		$sts;
		$stshdr;
		$des;
		...
		try {
			...
			$id_header = $data_json->xrow_header[0]->style_spec_header_id;
			$id_basic = $this->CI->main->find_data_pop('dbo.dt_style_spec_detail', 'style_spec_header_id', $id_header, 1);
			// validasi jika data kosong alert muncul
			//var_dump(count($id_basic));die();
			...
			//$this->CI->load->library('mainconfig');
			//  $value=$this->CI->main->get_absen_1($date,$name,$departemen);
			// $value=$this->CI->main->find_data_pop($table,$field,$keyword=null,$limit=null)
			...
			if ($format == 'pdf') {
				...
				$template_trims_data = 'C:/tmp_sipop/specification/excel/lap_spec_trims_data.fods';
				$template_history_data = 'C:/tmp_sipop/specification/excel/lap_spec_history_data.fods';
				$template_image_data = 'C:/tmp_sipop/specification/excel/lap_spec_image_data.fods';
				$tmp_ext = 'fods';
				$EXTENSION = 'xlsx';
				$CONTENT_TYPE = 'application/msexcel';
				//$CONTENT_TYPE='application/vnd.ms-excel';
			...
			$template_detail = file_get_contents($template_detail_data);
			$template_trims = file_get_contents($template_trims_data);
			$template_thistory = file_get_contents($template_history_data);
			$template_image = file_get_contents($template_image_data);
			// $template_data=file_get_contents($templateData);

			// variable template
			// $var_periode='{periode}';
			// $value_periode=$data_json->->tanggal;
			// $value_doc .=str_replace($var_periode,$value_periode,$template_doc);

			$doc_header = array('{number}', '{style}', '{buyer}', '{date}', '{po}', '{pattern}', '{note}', '{fabric}', '{allowance}', '{approve}', '{data_detail}', '{data_trims}');
			//$data_json->xrow_header[0]->style_spec_nomor
			$isi_header = array(
				$data_json->xrow_header[0]->style_spec_nomor,
				$data_json->xrow_header[0]->item_code,
			...
			foreach ($isi_header as $isi) {
				$dt_header .= $isi;
				// $dt_header.=str_replace($doc_header,'')
			};
			// var_dump($dt_header);die(); 
			//$value_doc=str_replace($doc_header,$isi_header,$template_doc);
			$value_header = str_replace($doc_header, $isi_header, $template_doc);
			$header_id = $data_json->xrow_header[0]->style_spec_header_id;
			$hsl_detail = $this->CI->main->spec_detail($header_id);
			...
			foreach ($hsl_detail as $content) {
				//foreach($hsl_detail as $element=>$content){
				// var_dump ((float)$content['r6']);die();	
				//$ceknilai = isset($_POST['purchase_order_id']) ? $_POST['purchase_order_id'] : '';
				if ($content['r5'] == '0.000000') {
					$content['r6'] = '';
					$content['r7'] = '';
					$content['r8'] = '';
			...
			//var_dump($value_doc);die();	
			$isi_header2 = $data_json->xrow_header[0]->style_spec_nomor;
			// ambil isi file template
			// $this->mainconfig->decimalToFraction($v);
			...
			//$value=array($isi_header2,$value_doc);

			$DOC = str_replace($doc_header, $dataheader, $template_doc);
			//var_dump($template_doc);die();
			//  $template_doc=file_get_contents($template.'lap_barang_jadi_2_data.fodt');
			// $var_doc=array('{no}','{kode_barang}','{nama_barang}');
			// $value_doc=array('1','TY1234/TST','Percobaan');
			...
			file_put_contents($temp . $report_name . $report_time . $tmp_ext, $DOC);
			//file_put_contents($temp.$report_name.$report_time.$tmp_ext,$valus_doc);
	...
	public function resultPrint_laporan($prepare = 'my_query', $withLog = false, $jsonformat = false, $die = false, $format, $date_start, $date_end)
	{
		...
		try {
			...
			foreach ($hasil as $element => $content) {
				...
				$r5 = $this->CI->mainconfig->get_decimal_format($content['r5'], 4, true);
				$r9 = $this->CI->mainconfig->get_decimal_format($content['r9'], 4, true);
				$value_doc .= str_replace($var_data, array($x, $content['r2'], $content['r3'], $content['r4'], $r5, $content['r6'], $r7, $r9, number_format($content['r8'], 4)), $template_data);
				//$value_doc.=str_replace($var_data,array($x,$content->r2,$content->r3,$content->r4,$content->r5,$content->r6,$content->r7,$content->r8),$template_data);
				$x = $x + 1;
			}
			// foreach($data_json->xrow as $element => $content){
	...
	public function resultPrint_sipop($prepare = 'my_query', $withLog = false, $jsonformat = false, $die = false)
	{
		...
		try {
			...
			$params_info = array('Param' => $this->encryptRJ256($param_text));
			$PORTAL_PORT = '8083';
			$url = "http://localhost:" . $PORTAL_PORT . "/ecc/http?wsdl";
			// $webService_info = $this->my_soap->ExecECC($params_info); 
			// $strXMLData_info = $webService_info->ExecECCResult; 


			// $result = $this->go2($request,$url); 
			$result = $this->go2($request, $this->url);
			var_dump($result);
			die();
			$doc_report = DOMDocument::loadXML(base64_decode($result));
	...
	function formatToXml_field($field)
	{
