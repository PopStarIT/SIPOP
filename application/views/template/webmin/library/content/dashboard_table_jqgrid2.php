...
//= jqgrid ini membedakan dari yang aslinya hanya di doubleclik saja
$colNames = array();
$model = array();
if (isset($field)) {
	foreach ($field as $key => $dt_field) {
		...
		if (isset($dt_field['editoptions'])) {
			$row2['value'] = $dt_field['editoptions'];
			//$row['editoptions'] ='{'.$row2[].'}';
			$row['editoptions'] = $dt_field["editoptions"];
			//$row['editoptions'] =listOptions;
			//var_dump($row['editoptions']);
		}
	...
	$(function() {
		...
		$("#table_<?php echo $table_id ?>").jqGrid({
			...
			ondblClickRow: function(id) {
				//console.log(lastSel_<?php echo $table_id ?>);
			  if(cek==1){
				   $(this).restoreRow(lastSel_<?php echo $table_id ?>);
				   cek=0;
				  // lastSel_<?php echo $table_id ?> = id;
			   }else{
				  //  jQuery('#table_<?php echo $methodid ?>_detail_receive').jqGrid('editRow',id,true);
				  $(this).editRow(id, true);
				   cek=1;
				  //lastSel_<?php echo $table_id ?> =0;
			   }
		...
		$("#table_<?php echo $table_id ?>").jqGrid("setColProp", "rn", {
			hidedlg: false
		});
		$("#table_<?php echo $table_id ?>").jqGrid('navGrid', '#ptable_<?php echo $table_id ?>', {
			edit: false,
			add: false,
			del: false,
			view: false,
		...
		$("#table_<?php echo $table_id ?>").jqGrid('filterToolbar', {
			stringResult: true,
			searchOnEnter: false,
			defaultSearch: 'cn',
			ignoreCase: false
		...
		$("#table_<?php echo $table_id ?>").bind("jqGridAfterLoadComplete", function() {
			...
			var namatabel="<?php echo $table_id ?>" ;
			 if(namatabel=='781145_detail_scan'){
               $cells.wrapInner("");
			   $colHeaders.wrapInner("");
			  }else if(namatabel=='781168_custom'){
			    $cells.wrapInner("");
			   $colHeaders.wrapInner(""); 
			 }else if(namatabel=='781145_receive_scan'){
			   $cells.wrapInner("");
			   $colHeaders.wrapInner(""); 
			 }else{
				$cells.wrapInner("<span class='mywrapping'></span>");
