<?php if(isset($dashboard_table)) { ?>	
	...
	//var_dump($dashboard_table['field_sub_detail']);die();
	...
	//var_dump($extra_data);
	if(isset($dashboard_table['field_sub_detail'])) {
		foreach($dashboard_table['field_sub_detail'] as $key => $dt_field){
			
			$colNames2[] = strtoupper(str_replace('_',' ',$dt_field['title']));
		...
		$colModel2 = json_encode($model);
	}
	 //var_dump($colModel2);die();
	  // var_dump($colNames2);die();
	  //$this->mainconfig->get_decimal_format($value[1],12);
	?>
	
	<script type="text/javascript">  
  // alert(baseurl+'<?php echo $class_uri ?>/loaddata_sub');
		$(function () {
			"use strict";
			$("#table_<?php echo $methodid ?>").jqGrid({
				...
				subGridRowExpanded: function(subgrid_id, row_id) {
		               ...
		               jQuery("#"+subgrid_table_id).jqGrid({
					       ...
					       url: baseurl+'<?php echo $class_uri ?>/loaddata_sub',
		                  	//datatype: "xml",
						   postData:{'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',row_id:row_id},
						   mtype : "post",
						   datatype: "json",
						  // colNames:[<?php echo "'". implode("','",$colNames2) ."'" ?>],
		                  	colNames: ['Item','Uraian','Qty'],
						//	colmodel:<?php echo $colModel2 ?>,
						  	colModel: [
		                  		//{name:"r1",index:"r1",width:80,key:true},
		                  		{name:"r3",index:"r3",width:130},
		                  		{name:"r4",index:"r4",width:200,align:"right",sortable:false},
		                  		{name:"r6",index:"r6",width:70,align:"right",sortable:false}
		                 ...
		                 // 	],
						  ...
						  jQuery("#"+subgrid_table_id).jqGrid("setColProp", "rn", {hidedlg: false});
						  jQuery("#"+subgrid_table_id).jqGrid('navGrid',"#"+pager_id,{edit:false,add:false,del:false})
		                 // jQuery("#"+subgrid_table_id).jqGrid('navGrid','#ptable_<?php echo $methodid ?>',{edit:false,add:false,del:false})
	            },
