<script type="text/javascript">
	$(function() {
		...
		$("#table_<?php echo $methodid ?>").jqGrid({
			...
			datatype: "json",
			colNames: ['TIPE BC', 'NO', 'TANGGAL', 'NO', 'TANGGAL', 'SUPPLIER NAME', 'KODE', 'NAMA', 'SAT', 'QTY', 'VAL', 'JML','CAR','HS CODE','UNIT RECEIVE','CONVERSION','QTY WAREHOUSE'],
			colModel: [{
					name: 'r1',
					index: 'r1',
					width: 60,
					...
					searchoptions: {
						searchhidden: true,
						value: ':ALL;BC 2.3:BC 2.3;BC 2.6.2:BC 2.6.2;BC 2.7:BC 2.7;BC 4.0:BC 4.0'
					}
				},
				{
					name: 'r2',
						...
						dataInit: function(el) {
							$(el).datepicker({
								onSelect: function(date) {
									jQuery("#table_<?php echo $methodid ?>")[0].triggerToolbar();
								},
								changeMonth: true,
								changeYear: true,
								format: 'yyyy-mm-dd',
		...
		$("#table_<?php echo $methodid ?>").jqGrid('setGroupHeaders', {
			...
			groupHeaders: [{
				startColumnName: 'r2',
				numberOfColumns: 2,
				titleText: '<em><center>DOC</center></em>'
			}, {
				startColumnName: 'r4',
				numberOfColumns: 2,
				titleText: '<em><center>RECEIVE</center></em>'
			}, {
				startColumnName: 'r7',
				numberOfColumns: 2,
				titleText: '<em><center>ITEM</center></em>'
			}]
		});
	...
	function print_<?php echo $methodid ?>(format) {
		...
		//alert(date_start);
               ...
               $.ajax({
                  type: "POST",
                  url:baseurl + '<?php echo $class_uri ?>/print_excel',
				  data: data_send,
                  dataType : 'json',
                  complete: function(){
                  },
