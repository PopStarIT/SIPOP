...
Class Custom_import extends CI_Controller { 
	...
	function custom_import_detail_table() {
		$view = 'view_custom_import_detail_new';
		$get_field = $this->ecc_library->get_field($view);
	...
	function approve(){
		...
		if(count($_POST) > 0){
			...
			if($bc_in_header_id){
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_approve");
	...
	function cancel_approve(){
		...
		if(count($_POST) > 0){
			...
			if($bc_in_header_id){
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_cancel_approve");
				$this->rpc_service_portal->addField('bc_in_header_id',$bc_in_header_id);
			}
	...
	function delete(){
		...
		if(count($_POST) > 0){
			...
			if($bc_in_header_id){
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_delete");
				$this->rpc_service_portal->addField('bc_in_header_id',$bc_in_header_id);
			}
	...
	function post_add_edit(){
		...
		if(count($_POST) > 0){
			if($bc_in_header_id){
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_edit_ceisa40");
				$this->rpc_service_portal->addField('bc_in_header_id',$bc_in_header_id);
			} else {
				switch($bc_in_type_id){
					case '1':
						$this->rpc_service_portal->setSP("dbo.sp_custom_import_add_ceisa40");
					break;
					
					case '2':
						$this->rpc_service_portal->setSP("dbo.sp_custom_import_add_from_performa_ceisa40");
					...
					case '3':
						$this->rpc_service_portal->setSP("dbo.sp_custom_import_add_from_purchase");
					...
					case '4':
						$this->rpc_service_portal->setSP("dbo.sp_custom_import_add_from_contract_subcon_ceisa40");
			...
			$this->rpc_service_portal->addField('bc_in_type_id',$bc_in_type_id);
			...
			$this->rpc_service_portal->addField('bongkar_kppbc_id',$bongkar_kppbc_id);			
			$this->rpc_service_portal->addField('tujuan_tpb_id',$tujuan_tpb_id);			
			...
			$this->rpc_service_portal->addField('tujuan_pemasukan_id',$tujuan_pemasukan_id);			
			$this->rpc_service_portal->addField('pjt_status_id',$pjt_status_id);			
			...
			$this->rpc_service_portal->addField('cifRupiah',$cifRupiah);			
			$this->rpc_service_portal->addField('value_additional',$value_additional);			
			$this->rpc_service_portal->addField('discount',$discount);			
			...
			$this->rpc_service_portal->addField('tps_id',$tps_id);			
			$this->rpc_service_portal->addField('muat_port_id',$muat_port_id);			
			$this->rpc_service_portal->addField('transit_port_id',$transit_port_id);			
			...
			$this->rpc_service_portal->addField('jumlah_tanda_pengaman',$jumlah_tanda_pengaman);			
			$this->rpc_service_portal->addField('kode_jenis_nilai',$kode_jenis_nilai);			
			...
			$this->rpc_service_portal->addField('tgl_bc11',$tgl_bc11);			
			...
			$this->rpc_service_portal->addField('tgl_perkiraan_tiba',$tgl_perkiraan_tiba);		
	...
	function post_add_edit_detail(){
		...
		if(count($_POST) > 0){
			if($bc_in_barang_id){
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_detail_edit_ceisa40");
				$this->rpc_service_portal->addField('bc_in_barang_id',$bc_in_barang_id);
			} else {
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_detail_add_ceisa40");
			}
			...
			$this->rpc_service_portal->addField('cif',$cif);
	...
	function post_add_edit_document_xx(){
		...
		if(count($_POST) > 0){
			if($id_dokumen){
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_document_in_edit");
				$this->rpc_service_portal->addField('id_dokumen',$id_dokumen);
			} else {
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_document_in_add");
			}
	...
	function post_add_edit_jaminan(){
		...
		if(count($_POST) > 0){
			if($idJaminan){
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_import_jaminan_edit");
				$this->rpc_service_portal->addField('idJaminan',$idJaminan);
			} else {
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_import_jaminan_add");
			}
	...
	function post_add_edit_kontainer(){
		...
		if(count($_POST) > 0){
			if($id_kontainer){
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_kontainer_in_edit");
				$this->rpc_service_portal->addField('id_kontainer',$id_kontainer);
			} else {
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_kontainer_in_add");
			}
	...
	function delete_detail(){
		...
		if(count($_POST) > 0){
			...
			if($bc_in_barang_id){
				$this->rpc_service_portal->setSP("dbo.sp_custom_import_detail_delete");
				$this->rpc_service_portal->addField('bc_in_barang_id',$bc_in_barang_id);
			}
	...
	function delete_document(){
		...
		if(count($_POST) > 0){
			...
			if($id_dokumen){
				$this->rpc_service_portal->setSP("dbo.sp_custom_ceisa_document_in_delete");
				$this->rpc_service_portal->addField('id_dokumen',$id_dokumen);
			}
	...
	function delete_kemasan(){
		...
		if(count($_POST) > 0){
			...
			if($idJaminan){
				$this->rpc_service_portal->setSP("dbo.sp_ceisa_custom_import_jaminan_delete");
				$this->rpc_service_portal->addField('idJaminan',$idJaminan);
			}
	...
	function delete_kontainer(){
		...
		if(count($_POST) > 0){
			...
			if($id_kontainer){
				$this->rpc_service_portal->setSP("dbo.sp_custom_in_ceisa_kontainer_delete");
				$this->rpc_service_portal->addField('id_kontainer',$id_kontainer);
			}
			...
			$dokumen++;
				...
				$q_pengangkut = $this->db->query('
					...
					SELECT  "bc_in_header_id"
					  ...
					  ,COALESCE("kodeCaraAngkut",'. "''" .') as "kodeCaraAngkut"
					  ,'. "'1'" .' as "seriPengangkut"
