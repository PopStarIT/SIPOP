<script type="text/javascript">  
	...
	function nav_button_<?php echo $function ?>(){
		...
		if (id) { 
			var row = jQuery("#table_<?php echo $methodid ?>").jqGrid('getRowData',id); 
				...
				$("#table_<?php echo $methodid ?>_responlogs").jqGrid('setGridParam',{postData:{'q':'1','bc_in_header_id':$("#txt_<?php echo $methodid ?>_header").val(),'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>'}}).trigger("reloadGrid"); 
	...
	$("#btnprint_log_<?php echo $methodid ?>").click(function(){
			...
			if (id) { 
				...
				window.open(baseurl+'<?php echo $class_uri ?>/cetak_respon_ceisa'+ '?bc_in_header_id=' + row.bc_in_header_id + '&id_respon='+row.id_respon + '&Pdf='+row.Pdf,'_BLANK');
				return false;
	...
	/* 
	 ...
	 $("#btnprint_log_<?php echo $methodid ?>").click(function(){
			var form_data = {bc_out_header_id:$("#bc_out_header_id_<?php echo $methodid ?>").val(), id_respon:$("#id_respon_<?php echo $methodid ?>").val(),'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>'}
			$.ajax({
				url : baseurl+'<?php echo $class_uri ?>/cetak_respon_ceisa',
				type : 'POST',
				dataType: 'json',
				data : form_data,
				success: function(msg){
					if (msg.sts == '00'){
						...
						$('#mdl_pengoloran1_<?php echo $methodid ?>').modal('toggle');
