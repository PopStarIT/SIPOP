<script type="text/javascript"> 
	...
	$(".form_tab_<?php echo $methodid ?>").on("click", "a", function (e) {
		...
		setTimeout(function(){ 
			...
			$("#table_<?php echo $methodid ?>_fixed_asset").trigger('reloadGrid');
			$("#table_<?php echo $methodid ?>_fixed_asset").setGridWidth($('.grid_container_<?php echo $methodid; ?>_fixed_asset').width() - 20,true).trigger('resize');
	...
	$('#form_<?php echo $methodid ?>_item_id_fixed_asset').on('change', function (event, clickedIndex, newValue, oldValue) {
		$("#table_<?php echo $methodid ?>_fixed_asset").trigger('reloadGrid');
	});
	...
	function print_<?php echo $methodid ?>_fixed_asset(format){
      item_id = $('#form_<?php echo $methodid ?>_item_id_fixed_asset').val();
	  ...
	  // } else {
		  ...
		  $.ajax({
			 type: "POST",
			 url:baseurl + '<?php echo $class_uri ?>/print_fixed_asset',
			 data: data_send,
			 dataType : 'json',
			 complete: function(){
			 },
