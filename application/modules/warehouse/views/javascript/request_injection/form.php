<script type="text/javascript">
	...
	var request_detail_id = 0;
	var request_supply_id = 0;
	
	//$(function () {
   //     "use strict";
		
	  $("#table_<?php echo $methodid ?>_cek_stock").jqGrid({
			url: baseurl+'<?php echo $class_uri ?>/loaddata_cek_stock',
			mtype : "post",
			postData:{'q':'1','date_start':'<?php echo date("Y-m-d", strtotime( date( "Y-m-d", strtotime( date("Y-m-d") ) ) . "-1 month" ) ) ?>','date_end':'<?php echo date("Y-m-d") ?>','<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>','request_id':'-1'},
			datatype: "json",
			colNames:['REQUEST ID','REQUEST LIST ID','REQUEST NO','ITEM BASE CODE','ITEM CODE', 'QUANTITY REQUEST','STOCK REPORT','STOCK_ASSETS','ASSETS GUDANG 2'],
			colModel:[
				{name:'r1',index:'r1', hidden: true},
				{name:'r2',index:'r2', hidden: true},
				{name:'r3',index:'r3', hidden: true},
				{name:'r4',index:'r4', width:100,align:'center'},  
				{name:'r5',index:'r5', width:100,align:'center'},  
				{name:'r6',index:'r6', width:100,align:'right',formatter:'formatNumerics'}, 
				{name:'r7',index:'r7', width:100,align:'right',formatter:'formatNumerics'},  
				{name:'r8',index:'r8', width:150,align:'right',formatter:'formatNumerics'},
				{name:'r9',index:'r9', width:150,align:'right',formatter:'formatNumerics'}
			],
			 iconSet: "fontAwesome",
            iconSet: "fontAwesome",
            idPrefix: "g1_",
			...
			rowNum:10,
			rowList:[10,20,30],
			pager: '#ptable_<?php echo $methodid ?>_cek_stock',
            sortname: "r1",
            sortorder: "asc",
			shrinkToFit:true,
			autowidth: true,
		...
		$("#table_<?php echo $methodid ?>_cek_stock").jqGrid("setColProp", "rn", {hidedlg: false});
		
	
		$("#table_<?php echo $methodid ?>_cek_stock").jqGrid('navGrid','#ptable_<?php echo $methodid ?>_cek_stock',{edit:false,add:false,del:false,view:false, search: false});  
		$("#table_<?php echo $methodid ?>_cek_stock").jqGrid('filterToolbar',{stringResult: true,searchOnEnter : false, defaultSearch: 'cn', ignoreCase: false});
	//});
	...
	function modal_cek_stock_<?php echo $methodid ?>(){
		var req_no= $('#form_<?php echo $methodid ?>_request_no').val();
		let request_id= $('#form_<?php echo $methodid ?>_detail_request_id').val();
		var request_detail_id= $('#form_<?php echo $methodid ?>_detail_request_detail_id').val();
		//alert(request_id);
		
		 $("#table_<?php echo $methodid ?>_cek_stock").jqGrid('setGridParam',{
			postData: {
			           	'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>'
			           	 ,request_id:request_id
			           	 ,date_start:'<?php echo date("Y-m-d", strtotime( date( "Y-m-d", strtotime( date("Y-m-d") ) ) . "-1 month" ) ) ?>'
			           	 ,date_end:'<?php echo date("Y-m-d") ?>'
			           } 
			    });
				
		$("#table_<?php echo $methodid ?>_cek_stock").trigger('reloadGrid');
		//$("#table_<?php echo $methodid ?>_tbl_cek_stock").setGridWidth($('.grid_container_<?php echo $methodid; ?>_tbl_cek_stock').width() - 20,true).trigger('resize');
		
		 
		 $('#form_<?php echo $methodid ?>_cek_stock_request_no').val(req_no);
		 $('#form_<?php echo $methodid ?>_cek_stock_request_id').val(request_id);
		 $('#form_<?php echo $methodid ?>_cek_stock_request_detail_id').val(request_detail_id);
		 $('#form_<?php echo $methodid ?>_cek_stock_date_start').val('<?php echo date("Y-m-d", strtotime( date( "Y-m-d", strtotime( date("Y-m-d") ) ) . "-1 month" ) ) ?>');
		 $('#form_<?php echo $methodid ?>_cek_stock_date_end').val('<?php echo date("Y-m-d") ?>');
		
		 $('#modal_cek_stock').modal('show')
	...
	function edit_<?php echo $methodid?>(id){
		$.ajax({
			...
			success: function(data){
				...
				$('.label_form_<?php echo $methodid ?>_detail_quantity_request').html('Quantity request');
			
				$('#form_<?php echo $methodid ?>_detail_quantity_request').val(0);
				
				$('#form_<?php echo $methodid ?>_request_id').val(request_id);
				$('#form_<?php echo $methodid ?>_request_no').val(data[0].request_no);
				$('#form_<?php echo $methodid ?>_request_date').val(data[0].request_date);
				$('#form_<?php echo $methodid ?>_info').val(data[0].info);
				
				$('#form_<?php echo $methodid ?>_detail_request_no').val(data[0].request_no);
				
				$('#form_<?php echo $methodid ?>_detail_request_id').val(request_id);
	...
	function edit_detail_<?php echo $methodid ?>(id){
		...
		$.ajax({
			...
			data: {
				'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>'
				,param: 'data_request_injection_detail'
				,id : id
			},
			dataType: 'json',
			method: 'post',
			success: function(data){
				...
				$('#form_<?php echo $methodid ?>_detail_request_detail_id').val(data[0].request_detail_id);
	...
	function cancel_detail_<?php echo $methodid ?>(){
		$('#form_<?php echo $methodid ?>_detail_request_detail_id').val('');
		$('#form_<?php echo $methodid ?>_detail_quantity_request').val(0);
	...
	var click_transfer_<?php echo $methodid ?> = function (id, isSelected) {
		...
		$('#form_<?php echo $methodid ?>_supply_request_supply_id').val('');
		...
		if (!isSelected) {
			$('#form_<?php echo $methodid ?>_supply_request_detail_id').val('');
		} else {
			var row = jQuery("#table_<?php echo $methodid ?>_supply").jqGrid('getRowData',id);
			request_detail_id = parseInt(unwrap_cell_value(row.r1).replace(/,/g, ''));
			$('#form_<?php echo $methodid ?>_supply_request_detail_id').val(request_detail_id);
		}
	...
	var click_item_<?php echo $methodid ?> = function (id, isSelected) {	
		if (!isSelected) {
			...
			var row_supply = $("#table_<?php echo $methodid ?>_list_transfer").jqGrid('getRowData',id);
			if(unwrap_cell_value(row_supply.r1) == id){
				request_supply_id = parseInt(unwrap_cell_value(row_supply.r3).replace(/,/g, ''));
			...
			$('#form_<?php echo $methodid ?>_supply_request_supply_id').val(request_supply_id);
	...
	var click_supply_<?php echo $methodid ?> = function (id, isSelected) {
		if (!isSelected) {	
			...
			request_supply_id = parseInt(unwrap_cell_value(row.r3).replace(/,/g, ''));
			quantity_supply = parseFloat(unwrap_cell_value(row.r9).replace(/,/g, ''));
	...
	function supply_<?php echo $methodid?>(id){
		$.ajax({
			...
			success: function(data){
				...
				$('#form_<?php echo $methodid ?>_supply_request_id').val(request_id);
				$('#form_<?php echo $methodid ?>_supply_request_detail_id').val('');
				$('#form_<?php echo $methodid ?>_supply_request_no').val(data[0].request_no);
				$('#form_<?php echo $methodid ?>_supply_request_date').val(data[0].request_date);
				$('#form_<?php echo $methodid ?>_supply_info').val(data[0].info);
			...
			swal({
				...
				if (result.value) {
					...
					$.ajax({
						...
						data: {'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>',request_id:request_id},
						dataType: 'json',
						method: 'post',
						success: function(data){
							page_loading("hide");
	...
	function post_<?php echo $methodid ?>_upload(){
		if(check_submit == 0) {
			...
			var file_data = $('#userfile_<?php echo $methodid ?>_request_inventory').prop('files')[0];  
			var data = $("#form_<?php echo $methodid ?>_upload").serializeArray();
			form_data.append('userfile_<?php echo $methodid ?>_request_inventory', file_data);
