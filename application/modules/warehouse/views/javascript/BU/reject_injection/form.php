<script type="text/javascript">
	var new_reject_injection = 1;
	var reject_injection_id = 0;
	var reject_injection_quantity = 0;
	var reject_injection_detail_id = 0;
	var reject_injection_supply_id = 0;
	...
	function edit_<?php echo $methodid?>(id){
		$.ajax({
			...
			data: {
				...
				,param: 'data_reject_injection'
				,id : id
			},
			dataType: 'json',
			method: 'post',
			success: function(data){
				reject_injection_id = data[0].reject_injection_id;
				new_reject_injection = 0;				
								
				$('.label_form_<?php echo $methodid ?>_detail_quantity_reject_injection').html('Quantity Reject injection');
			
				$('#form_<?php echo $methodid ?>_detail_quantity_reject_injection').val(0);
				
				$('#form_<?php echo $methodid ?>_reject_injection_id').val(reject_injection_id);
				$('#form_<?php echo $methodid ?>_reject_injection_no').val(data[0].reject_injection_no);
				$('#form_<?php echo $methodid ?>_reject_injection_date').val(data[0].reject_injection_date);
				$('#form_<?php echo $methodid ?>_reject_injection_info').val(data[0].reject_injection_info);
				
				$('#form_<?php echo $methodid ?>_detail_reject_injection_id').val(reject_injection_id);
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.valid){
						...
						if(new_reject_injection == 1){
							new_reject_injection = 0;
							$("#tab_<?php echo $methodid; ?>_detail").attr("data-toggle","tab");
							$("#tab_<?php echo $methodid; ?>_detail").removeClass( "tab_disabled");
							$("#tab_<?php echo $methodid; ?>_detail").click();
							reject_injection_id = data.reject_injection_id;
							$('#form_<?php echo $methodid ?>_reject_injection_id').val(reject_injection_id);
							$('#form_<?php echo $methodid ?>_detail_reject_injection_id').val(reject_injection_id);
	...
	function edit_detail_<?php echo $methodid ?>(id){
		...
		$.ajax({
			...
			data: {
				'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>'
				,param: 'data_reject_injection_detail'
				,id : id
			},
			dataType: 'json',
			method: 'post',
			success: function(data){
				...
				$('#form_<?php echo $methodid ?>_detail_reject_injection_detail_id').val(data[0].reject_injection_detail_id);
				$('#form_<?php echo $methodid ?>_detail_quantity_reject_injection').val(data[0].quantity_reject_injection);
	...
	function cancel_detail_<?php echo $methodid ?>(){
		$('#form_<?php echo $methodid ?>_detail_reject_injection_detail_id').val('');
		$('#form_<?php echo $methodid ?>_detail_quantity_reject_injection').val(0);
	...
	var click_transfer_<?php echo $methodid ?> = function (id, isSelected) {
		...
		$('#form_<?php echo $methodid ?>_supply_reject_injection_supply_id').val('');
		...
		if (!isSelected) {
			$('#form_<?php echo $methodid ?>_supply_reject_injection_detail_id').val('');
		} else {
			var row = jQuery("#table_<?php echo $methodid ?>_supply").jqGrid('getRowData',id);
			reject_injection_detail_id = parseInt(unwrap_cell_value(row.r1).replace(/,/g, ''));
			$('#form_<?php echo $methodid ?>_supply_reject_injection_detail_id').val(reject_injection_detail_id);
		}
	...
	var click_item_<?php echo $methodid ?> = function (id, isSelected) {	
		if (!isSelected) {
			...
			reject_injection_supply_id = '';
			
			var row_supply = $("#table_<?php echo $methodid ?>_list_transfer").jqGrid('getRowData',id);
			if(unwrap_cell_value(row_supply.r1) == id){
				reject_injection_supply_id = parseInt(unwrap_cell_value(row_supply.r3).replace(/,/g, ''));
			...
			$('#form_<?php echo $methodid ?>_supply_reject_injection_supply_id').val(reject_injection_supply_id);
	...
	var click_supply_<?php echo $methodid ?> = function (id, isSelected) {
		if (!isSelected) {	
			...
			reject_injection_supply_id = parseInt(unwrap_cell_value(row.r3).replace(/,/g, ''));
			quantity_supply = parseFloat(unwrap_cell_value(row.r9).replace(/,/g, ''));
	...
	function supply_<?php echo $methodid?>(id){
		$.ajax({
			...
			success: function(data){
				...
				$('#form_<?php echo $methodid ?>_supply_reject_injection_id').val(reject_injection_id);
				$('#form_<?php echo $methodid ?>_supply_reject_injection_detail_id').val('');
				$('#form_<?php echo $methodid ?>_supply_reject_injection_no').val(data[0].reject_injection_no);
				$('#form_<?php echo $methodid ?>_supply_reject_injection_date').val(data[0].reject_injection_date);
				$('#form_<?php echo $methodid ?>_supply_reject_injection_info').val(data[0].reject_injection_info);
			...
			swal({
				...
				if (result.value) {
					...
					$.ajax({
						...
						data: {'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>',reject_injection_id:reject_injection_id},
						dataType: 'json',
						method: 'post',
						success: function(data){
							page_loading("hide");
