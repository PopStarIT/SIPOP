<script type="text/javascript">  
	$(function () {
        ...
        $("#table_<?php echo $methodid ?>").jqGrid({
			...
			mtype : "post",
			postData:{'q':'1','date_start':'<?php echo date("Y-m-d", strtotime( date( "Y-m-d", strtotime( date("Y-m-d") ) ) . "-1 day" ) ) ?>','date_end':'<?php echo date("Y-m-d") ?>','<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>'},
			datatype: "json",
			colNames:[
				'TANGGAL'
				, 'NOMOR'
			...
			colModel:[
				...
				{name:'r5',index:'r5', width:150,align:'right'},  
				{name:'r6',index:'r6', width:150,align:'right'},  
				{name:'r7',index:'r7', width:150,align:'right'},
				{name:'r8',index:'r8', width:150,align:'right'},  
			],
			 iconSet: "fontAwesome",
            iconSet: "fontAwesome",
            idPrefix: "g1_",
		...
		$("#table_<?php echo $methodid ?>").jqGrid('setGroupHeaders', 
			...
			{startColumnName: 'r1', numberOfColumns: 2, titleText: '<em><center>TRANSAKSI</center></em>'},
			{startColumnName: 'r3', numberOfColumns: 2, titleText: '<em><center>DOKUMEN BEACUKAI</center></em>'},
		] });
