...
Class Mutasi_barang_jadi extends CI_Controller { 
	...
	function index(){
		...
		$component['view_load'] = 'report/mutasi_barang_jadi/view';
		$component['load_js'][] = 'report/mutasi_barang_jadi/view';
		
		$component['page_title'] = "Mutasi Barang Jadi Warehouse 1";
	...
	function loaddata(){
		...
		$sp = "dbo.sp_rpt_mutasi_barang_jadi_warehouse";		
		if($print == 1){
	...
	function print_xls(){
		...
		$phpExcel = PHPExcel_IOFactory::load(FCPATH."/assets/warehouse/MUTASI_BARANG_JADI.xlsx");	
		...
		$q = $this->db->query("
			select 
				...
				WHERE i.custom_item_type_id =2 and (c.quantity>0 OR (COALESCE(c.quantity,0) + COALESCE(d.quantity,0) + COALESCE(e.quantity,0) + COALESCE(f.quantity,0)>0) )
		...
		$filename =  "MUTASI_barang_jadi_"  . $date_start . '_' . $date_end;
