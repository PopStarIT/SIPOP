...
Class Mutasi_barang_jadi_wh2 extends CI_Controller { 
	...
	function mutasi_barang_jadi_table(){
		$field = array();
	...
	function index(){
		...
		$component['view_load'] = 'report/mutasi_barang_jadi_wh2/view';
		$component['load_js'][] = 'report/mutasi_barang_jadi_wh2/view';
		
		$component['page_title'] = "Mutasi Barang Jadi Warehouse 2";
		$dashboard_table = array();
	
		$field = $this->mutasi_barang_jadi_table();
	...
	function loaddata(){
		...
		$sp = "dbo.sp_rpt_mutasi_barang_jadi_warehouse_2";		
		if($print == 1){
	...
	function print_xls(){
		...
		$phpExcel = PHPExcel_IOFactory::load(FCPATH."/assets/warehouse/MUTASI_BARANG_JADI_WH2.xlsx");	
		...
		$q = $this->db->query("
			select 
				...
				, b.uom_code
				, COALESCE(j.quantity,0) as saldo_awal 
				, COALESCE(k.quantity,0) as pemasukan
				, COALESCE(l.quantity,0) * -1 as pengeluaran
				, COALESCE(m.quantity,0) as penyesuaian
				, COALESCE(j.quantity,0) + COALESCE(k.quantity,0) + COALESCE(l.quantity,0) + COALESCE(m.quantity,0) as saldo_akhir
				...
				WHERE i.custom_item_type_id =2 and ((j.quantity>0 OR (COALESCE(j.quantity,0) + COALESCE(k.quantity,0) + COALESCE(l.quantity,0) + COALESCE(m.quantity,0))>0))
		...
		$filename =  "MUTASI_barang_jadi_WH2_"  . $date_start . '_' . $date_end;
