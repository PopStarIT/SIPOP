...
Class Pengeluaran_wh1_wh2 extends CI_Controller { 
	...
	function index(){
		...
		$component['view_load'] = 'report/pengeluaran_wh1_wh2/view';
		$component['load_js'][] = 'report/pengeluaran_wh1_wh2/view';
		
		$component['page_title'] = "Pengeluaran Warehouse 1 ke warehouse 2";
	...
	function loaddata(){
        ...
        $sp = "dbo.sp_rpt_pengeluaran_wh1_wh2";			
		if($print == 1){
	...
	function print_xls(){
		...
		$phpExcel = PHPExcel_IOFactory::load(FCPATH . "/assets/template/warehouse/Pengeluaran_wh1_wh2.xlsx");
		$phpExcel->setActiveSheetIndex(0);  //set first sheet as active

		//$this->db_pop = $this->load->database('pop', TRUE);
		$q = $this->db->query("SELECT a.warehouse_transfer_id,
        a.warehouse_transfer_no,
        a.warehouse_transfer_date,
        b.request_no AS warehouse_request_no,
        b.request_date,
		i.item_code,
		i.item_name,
		h.quantity_supply,
		j.uom_code AS unit,
		h.item_id		
        FROM dbo.dt_warehouse_transfer a
        LEFT JOIN dbo.dt_request b ON a.request_id = b.request_id
        LEFT JOIN dbo.prm_warehouse_transfer_status d ON a.warehouse_transfer_status_id = d.warehouse_transfer_status_id
        LEFT JOIN dbo.dt_user e ON a.create_user_id = e.user_id
        LEFT JOIN dbo.dt_user f ON a.edit_user_id = f.user_id
        LEFT JOIN dbo.dt_user g ON b.edit_user_id = g.user_id
		 left JOIN dbo.dt_warehouse_transfer_detail h ON a.warehouse_transfer_id=h.warehouse_transfer_id
		 LEFT JOIN dbo.dt_mst_item i ON h.item_id = i.item_id
         LEFT JOIN dbo.ref_uom j ON i.uom_id = j.uom_id
		 WHERE (a.injection = 1::bit OR a.assembly = 1 ::bit ) AND (a.warehouse_transfer_date >= '$date_start' AND a.warehouse_transfer_date <= '$date_end')");
		...
		foreach ($q->result() as $r){
			...
			$phpExcel->getActiveSheet()->setCellValueExplicit("B".$j, $r->warehouse_transfer_no, PHPExcel_Cell_DataType::TYPE_STRING);
			$phpExcel->getActiveSheet()->setCellValueExplicit("C".$j, $r->warehouse_transfer_date);
			$phpExcel->getActiveSheet()->setCellValueExplicit("D".$j, $r->warehouse_request_no, PHPExcel_Cell_DataType::TYPE_STRING);
			$phpExcel->getActiveSheet()->setCellValueExplicit("E".$j, $r->request_date);
			$phpExcel->getActiveSheet()->setCellValueExplicit("F".$j, $r->item_code, PHPExcel_Cell_DataType::TYPE_STRING);
			$phpExcel->getActiveSheet()->setCellValueExplicit("G".$j, $r->item_name, PHPExcel_Cell_DataType::TYPE_STRING);
			$phpExcel->getActiveSheet()->setCellValue("H".$j, $r->quantity_supply);
			$phpExcel->getActiveSheet()->setCellValueExplicit("I".$j, $r->unit, PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$filename =  "Pengeluaran_wh1_ke_wh2_"  . $date_start . '_' . $date_end;
