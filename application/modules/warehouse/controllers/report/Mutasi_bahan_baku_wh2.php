...
Class Mutasi_bahan_baku_wh2 extends CI_Controller { 
	...
	function index(){
		...
		$component['view_load'] = 'report/mutasi_bahan_baku_wh2/view';
		$component['load_js'][] = 'report/mutasi_bahan_baku_wh2/view';
		
		$component['page_title'] = "Mutasi Bahan Baku dan Bahan Penolong Warehouse 2";
	...
	function loaddata(){
		...
		$sp = "dbo.sp_rpt_mutasi_bahan_baku_warehouse_2";		
		if($print == 1){
	...
	function print_xls(){
		...
		$phpExcel = PHPExcel_IOFactory::load(FCPATH."/assets/warehouse/MUTASI_BAHAN_BAKU_WH2.xlsx");	
		...
		$q = $this->db->query("
			select 
				...
				WHERE i.custom_item_type_id =1 and ((j.quantity>0 OR (COALESCE(j.quantity,0) + COALESCE(k.quantity,0) + COALESCE(l.quantity,0) + COALESCE(m.quantity,0))>0))
		...
		$filename =  "MUTASI_BAHAN_BAKU_WH2_"  . $date_start . '_' . $date_end;
