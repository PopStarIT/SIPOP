<script type="text/javascript">  
	$(function () {
        ...
        $("#table_<?php echo $methodid ?>").jqGrid({
			...
			postData:{'q':'1','date_start':'<?php echo date("Y-m-d", strtotime( date( "Y-m-d", strtotime( date("Y-m-d") ) ) . "1900-01-01" ) ) ?>','date_end':'<?php echo date("Y-m-d") ?>','<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>'},
			datatype: "json",
			colNames:['KODE', 'NAMA', 'SAT', 'QTY', 'KETERANGAN'],
			colModel:[ 
				{name:'r1',index:'r1', width:150},
				{name:'r2',index:'r2', width:300},  
				{name:'r3',index:'r3', width:40,align:'center'},  
				{name:'r4',index:'r4', width:120,align:'right',formatter:'formatNumerics'},  
