...
//alert("coba view");
   ...
   var karyawan_open_form =0;
	...
	$('#form_<?php echo $methodid ?>_cari_karyawan ').on('change', function (event, clickedIndex, newValue, oldValue) {
		cari_id = $('#form_<?php echo $methodid ?>_cari_karyawan').val();
		$.ajax({
				url: baseurl+'loader',
				data: {
					'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>'
					,param: 'get_cari_karyawan'
					,param_pop:'db_pop'
					,karyawan_id : cari_id
				...
				//	console.log(data);
					...
					$('#form_<?php echo $methodid ?>_nama_karyawan').val(data.karyawan_name);
					$('#form_<?php echo $methodid ?>_nik').val(data.karyawan_nik);
					$('#form_<?php echo $methodid ?>_departemen').val(data.karyawan_departemen);
					change_form_<?php echo $methodid ?>_divisi_id(data.divisi);
					change_form_<?php echo $methodid ?>_departemen_id(data.departemen);
					
					let gambar = new Image(); 
                     gambar.src = data.link_photo;
					$('#form_<?php echo $methodid ?>_link_photo_karyawan').val(data.link_photo);
					document.querySelector(".gbrphoto2").innerHTML='<img src="' + gambar.src + '" height="170px" width="150px" class="rouded-circle img-thumbnail mt-2 gbrphoto"  id="gambar"/>'; 
					//document.getElementById('gambar').innerHTML = '<img src="' + data.link_photo + '" />'; 
				}
		});
	...
	function cancel_<?php echo $methodid ?>_temp_idcard(){
		//alert('cancel');
		$('#panel_content_<?php echo $methodid ?>').show();
		$('#panel_content_form_template_<?php echo $methodid ?>').hide();
		
		$('#content_form_<?php echo $methodid ?>_template').hide(); 
	}
	...
	function save_<?php echo $methodid ?>_edit(){
		$('#form_<?php echo $methodid ?>_edit').submit();
	}
		...
		if(check_submit == 0) {
			...
			$.ajax({
				...
				success: function(data){
					...
					//	$('#form_<?php echo $methodid ?>_card_id').val(data.card_id);
						
				$('#panel_content_<?php echo $methodid ?>').show();
	        	$('#panel_content_form_<?php echo $methodid ?>').hide();
								
				$("#table_<?php echo $methodid ?>_id_card").trigger('reloadGrid');
		        $("#table_<?php echo $methodid ?>_id_card").setGridWidth($('.grid_container_<?php echo $methodid; ?>_id_card').width() - 20,true).trigger('resize');
	...
	function post_<?php echo $methodid ?>_edit(){
		...
		if(check_submit == 0) {
			...
			var data2 =  $("#form_<?php echo $methodid ?>_edit").serializeArray();
			
			$.each(data2, function (key,input) {
				frmdata.append(input.name,input.value);
			});
			// console.log(frmdata);	
			const file_data = $('#form_<?php echo $methodid ?>_edit_link_photo_edit').prop('files')[0];
   ...
   function add_<?php echo $methodid ?>_keluarga(){
	...
	function pictureadd_edit(){
	
		const filegbr= document.getElementById("form_<?php echo $methodid; ?>_edit_link_photo_edit");
		...
		//	$("#nmficture").val("");
			return false;
		      ...
		      //alert('asssss4');	
		      const gbr=document.querySelector(".gbrphoto_edit");
		      filegbr.textContent = name;
