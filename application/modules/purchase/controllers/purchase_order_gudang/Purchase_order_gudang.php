...
Class Purchase_order_gudang extends CI_Controller { 
   ...
   { 
	...
	function purchase_order_detail_table() {
		...
		$get_field['act']['formatter'] = 'formatOperations_po_sipop';
		$get_field['act']['width'] = 300;
		
		return $get_field;
	}
	
	function purchase_order_detail_rincian_table() {
		$view = 'view_purchase_order_warehouse_detail_rincian';
		$get_field = $this->ecc_library->get_field_pop($view);
		...
		$get_field['act']['sortable'] = false;
		//$get_field['act']['formatter'] = 'formatOperations';
		$get_field['act']['formatter'] = 'formatOperations_po_rincian';
		$get_field['act']['width'] = 100;
	...
	function index() {
		...
		$component['view_load'] = 'purchase_order_gudang/view';
		$component['view_load_form'] = 'purchase_order_gudang/form';
		$component['load_js'][] = 'purchase_order_gudang/view';
		$component['load_js'][] = 'purchase_order_gudang/form';
		
		$component['page_title'] = "Purchase Order Gudang";
		
		$dashboard_table = array();
		
		$nav_button = array();
		$nav_button[] = array('method_id' => 781131,'title' => 'Add','btn'=>'btn-success', 'icon' => 'fa fa-plus', 'load' => 'purchase_order_gudang/function_add');
		$nav_button[] = array('method_id' => 781132,'title' => 'Edit', 'btn'=>'btn-warning','icon' => 'fa fa-pencil', 'load' => 'purchase_order_gudang/function_edit');
		$nav_button[] = array('method_id' => 781133,'title' => 'Delete','btn'=>'btn-danger','icon' => 'fa fa-trash-o', 'load' => 'purchase_order_gudang/function_delete');
		$nav_button[] = array('method_id' => 781135,'title' => 'Approve', 'btn'=>'btn-info','icon' => 'fa fa-thumbs-up', 'load' => 'purchase_order_gudang/function_approve');
		$nav_button[] = array('method_id' => 781134,'title' => 'Cancel Approve','btn'=>'btn-danger','icon' => 'fa fa-thumbs-down', 'load' => 'purchase_order_gudang/function_cancel_approve');
		$nav_button[] = array('method_id' => 781114,'title' => 'Preview','btn'=>'btn-primary','icon' => 'fa fa-search', 'load' => 'purchase_order_gudang/function_print_warehouse');
		$nav_button[] = array('method_id' => 781118,'title' => 'Download Excel','btn'=>'btn-warning','icon' => 'fa fa-download', 'load' => 'purchase_order_gudang/function_download_warehouse');
		//$nav_button[] = array('method_id' => 781131,'title' => 'Add from Purchase order ECC','btn'=>'btn-success','icon' => 'fa fa-plus', 'load' => 'purchase_order_gudang/function_add_ecc');
		
		//$nav_button[] = array('method_id' => 781118,'title' => 'Update ETD/ETA', 'icon' => 'fa fa-pencil', 'load' => 'purchase_order_warehouse/function_edit');
	
		//  $user_id = $this->session->userdata('user_id');user_group
		//  var_dump($user_id);
		$field = $this->purchase_order_table();
		$field_detail = $this->purchase_order_detail_table();
		$field_detail_rincian = $this->purchase_order_detail_rincian_table();
		...
		$dashboard_table['field_list_detail_po'] = $field_detail_rincian;
		$dashboard_table['field_list_loaddata_detail_po'] = 'loaddata_detail_rincian';
		
		$dashboard_table['caption'] = '.:: Purchase Order Gudang ::.';
	...
	function delete()
   {
	...
	function post_add_edit() {
	    ...
	    // var_dump($purchase_order_no .' hasilnya');die();
		...
		if(count($_POST) > 0){
			...
			$this->rpc_service->addField('ppn',$ppn);
			 ...
			 // --- input data untuk data sipop ---------
	                         // var_dump($ket);die();
             $result = $this->rpc_service->resultJSON();	 
             // $result = array('valid'=>true,'data'=>true,'no'=>'','des'=>'ok');
			  //$data_result['purchase_order_id']=13795;
			  $data = array();
	            		        if(isset($result)){
	          			         if(isset($result['valid'])){
	          				      ...
	          				      if($result['valid']){
	          					   if(isset($result['data'])){
	          						  $data_result = json_decode($result['data'],true); // jika ingin test tanpa simpen ke ecc ini harus di comment
	          						        						 
			                          //====== Simpan data purchase warehouse ==========
									   //var_dump($data_result['purchase_order_id']);die();
									    $tanggal=date("Y-m-d H:i:s.u");
			                             ...
			                             $data_po_warehouse =array(
											...
											'date_eta'=>$date_eta,
											'date_in'=>$date_in,
											'purchase_order_no_warehouse'=>$purchase_order_no,
											'purchase_order_date_warehouse' =>$purchase_order_date,
	...
	function loaddata_detail_rincian() {
		$this->authentication->plainlayout();
		
	    $po_warehouse_detail_id = isset($_REQUEST['purchase_order_warehouse_detail_id']) ? is_numeric($_REQUEST['purchase_order_warehouse_detail_id']) ? $_REQUEST['purchase_order_warehouse_detail_id']  : -1 : -1;
		$methodid = isset($_REQUEST['methodid']) ? is_numeric($_REQUEST['methodid']) ? $_REQUEST['methodid']  : -1 : -1;
		
		$view = 'view_purchase_order_warehouse_detail_rincian';
		$field = $this->purchase_order_detail_rincian_table();
		
		$extra_param = array();
		$extra_param['where']['0']['field'] = 'r2';
		$extra_param['where']['0']['data'] = $po_warehouse_detail_id;
		$extra_param['methodid'] = $methodid;
	...
	function post_add_edit_detail() {
		...
		$quantity_ordered = isset($_POST['quantity_ordered']) ? $_POST['quantity_ordered'] : '0';
		...
		$conversion = isset($_POST['conversion']) ? $_POST['conversion'] : '1';
		$unit_price = isset($_POST['unit_price']) ? $_POST['unit_price'] : 1;
		...
		$po_content = isset($_POST['po_content']) ? $_POST['po_content'] : '';
		$color_code = isset($_POST['color_code']) ? $_POST['color_code'] : '';
		...
		if(count($_POST) > 0){
			...
			//$result = array('valid'=>true,'data'=>true,'no'=>'','des'=>'ok');
			//$data_result['purchase_order_id']=13795;
			//$data['purchase_order_detail_id']=35525;
							...
							$return['message'] = $result['des'];
							 ...
							 // ----Input Detail data warehouse ----------
							  ...
							  $data_PO_detail=array(
							   ...
							   'note'=>'',
							   'item_fabric_id' =>null,
							   'uom_id_warehouse_detail' =>$uom_id,
							   'conversion_warehouse_detail'=>$conversion,
							   'unit_price_warehouse_detail' =>$unit_price,
							   'quantity_ordered_warehouse_detail' =>$quantity_ordered
							 );
							 
							 	if($purchase_order_warehouse_detail_id){
									    $this->rpc_service->setSP("dbo.sp_purchase_order_warehouse_detail_edit");
				                        $this->rpc_service->addField('purchase_order_warehouse_detail',$purchase_order_warehouse_detail_id);
							      ...
							      //  $in_detail= $this->main->insert_pop2("dbo.dt_purchase_order_warehouse_detail",$data_PO_detail);
								   $this->rpc_service->setSP("dbo.sp_purchase_order_warehouse_detail_add");
							    }
								...
								$this->rpc_service->addField('purchase_order_detail_id',$purchase_order_detail_id);
			                    $this->rpc_service->addField('item_id',$item_id);
								$this->rpc_service->addField('note','');
								$this->rpc_service->addField('item_fabric_id','');
								$this->rpc_service->addField('uom_id_warehouse_detail',$uom_id);
								$this->rpc_service->addField('conversion_warehouse_detail',$conversion);
								$this->rpc_service->addField('unit_price_warehouse_detail',$unit_price);
								$this->rpc_service->addField('quantity_ordered_warehouse_detail',$quantity_ordered);
			                    $this->rpc_service->addField('order_delivery_warehouse_detail_date',$order_delivery_date);
								$this->rpc_service->addField('colour',$colour);
								$this->rpc_service->addField('po_content',$po_content);
								$this->rpc_service->addField('color_code',$color_code);
												
			                    
			                    $result_detail = $this->rpc_service->resultJSON_pop();
								$data_detail = array();
								
			                    if(isset($result_detail)){
								   if(isset($result_detail['valid'])){
									   if($result_detail['valid']){
										   if(isset($result_detail['data'])){
							                   $data = json_decode($result_detail['data'],TRUE);
							
							                   $return['valid'] = $result_detail['valid'];
							                   $return['status_code'] = $result_detail['no'];
							                   $return['message'] = $result_detail['des'];
										   }
									   }else {
					                      $return['status_code'] = $result_detail['no'];
					                   	  $return['message'] = $result_detail['des'];
	...
	function post_add_edit_detail_rincian() {
		$this->load->model('main');
		$this->authentication->plainlayout();
		$parameter = array();
		$return = array();
		...
		//======== Jika fabric_code=-99 atau item_fabric_code tidak kosong maka data akan dimasukan ke tabel dt_mst_item_fabric =====
		$item_fabric_code = isset($_POST['item_fabric_code']) ? $_POST['item_fabric_code'] : '';
		$item_fabric_name = isset($_POST['item_fabric_name']) ? $_POST['item_fabric_name'] : '';
		
	   if ($fabric_code == -99 or $fabric_code == null){
			if ($item_fabric_code AND $item_fabric_name){
				$item_fabric=array(
		         'item_fabric_code' => $item_fabric_code,
				 'item_fabric_name' => $item_fabric_name,
				 'item_id'=>$item_id,
		        );
			   $insert= $this->main->insert_pop2("dbo.dt_mst_item_fabric",$item_fabric);
			    if ($insert['pesan']){
					$item_fabric_id = $insert['id_data'];
			  	 					
				    $item_rincian=array(
		              'purchase_order_warehouse_detail_id' => $purchase_order_warehouse_detail_id,
		              'purchase_order_warehouse_id' => $purchase_order_warehouse_id,
		              'item_id' => $item_id,
				      'item_fabric_id' => $item_fabric_id,
				      'quantity_ordered_rincian' => $quantity,
		              'unit_price_rincian' => $unit_price,
		              'note' => $note
		            );
				     
				     $insert_rincian= $this->main->insert_pop2("dbo.dt_purchase_order_warehouse_detail_rincian",$item_rincian);
				     if ($insert_rincian['pesan']){
						 $des='Data berhasil disimpan ';
			             $val=true;
			             $no='00'; 
					 }
			  	}else{
				  $val=false;
				  $des='item fabric gagal disimpan.';
				  $no='01';
				}
			 			  
			}else{
			    $val=false;
				$des='Fabric Code/fabric Name Masih Kosong.';
				$no='01';
			}
		}else{
			  $item_rincian=array(
		     ...
		     $insert_rincian= $this->main->insert_pop2("dbo.dt_purchase_order_warehouse_detail_rincian",$item_rincian);
			    ...
			    if ($insert_rincian['pesan']){
				   ...
				   $val=false;
			       $des='Data gagal disimpan.';
			       $no='01';
				}
		}
		
		$result = array('valid'=>$val,'data'=>true,'no'=>$no,'des'=>$des);
	...
	function download_purchase_order_warehouse(){
	  ...
	  $purchase_order_id = isset($_REQUEST['purchase_order_id']) ? $_REQUEST['purchase_order_id'] : false;
	  $user_id = $this->session->userdata('user_id');
	  
	   if ($format =='xlsx'){
		   $this->download_purchase_order_excel();
	   }else{
	...
	function download_purchase_order_warehouse_old(){ 
	  $this->db_pop = $this->load->database('pop',TRUE);
	  $this->load->model('main');
       ...
       $template_data=file_get_contents($templateData);
		...
		//var_dump($q2->result());die();
		  foreach($q2->result() as $r2){
			...
			$isi_detail=array($r2->colour,$r2->color_code,number_format($r2->quantity_ordered,4),$qty,$amount,$r2->purchase_order_detail_memo,$r2->item_code);
	...
	function download_purchase_order_excel(){
	   			  
		require_once APPPATH."/third_party/PHPExcel.php"; 
		require_once APPPATH."/third_party/PHPExcel/IOFactory.php"; 
		
		$phpExcel = PHPExcel_IOFactory::load(FCPATH."/assets/template/purchase_order/fabric_purchase_order_excel.xlsx");	
	...
	$style_merge_wrap = [
      ...
      'alignment' => [
        ...
        'wrap'       => true // Ini untuk mengaktifkan Wrap Text
      ]
    ];
    ...
    'alignment' => [
        'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER, // Tengah samping
        'vertical'   => PHPExcel_Style_Alignment::VERTICAL_CENTER,   // Tengah atas-bawah
        'wrap'       => true                                         // Potong teks jika kepanjangan
    ],
    'borders' => [
        'allborders' => [
            'style' => PHPExcel_Style_Border::BORDER_THIN
    ...
    'borders' => [
	 ...
	 $purchase_order_id = isset($_REQUEST['purchase_order_id']) ? $_REQUEST['purchase_order_id'] : false;
	  ...
	  if (isset($row)){
			...
			$date_etd = $row->date_etd;
			$date_eta = $row->date_eta;
			$date_in = $row->date_in;
			$weight = $row->weight;
			$width = $row->width;
			$w = $row->w;
			...
			$packing_instructions = $row->packing_instructions;
			$shipping_sample = $row->shipping_sample;
			$requested_etd = $row->requested_etd;
	   ...
	   $phpExcel->getActiveSheet()->setCellValueExplicit("G2",$purchase_order_no, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('G2')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("G7",$purchase_order_date, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('G7')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->mergeCells('A12:D12');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("A12",$partner_name, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('A12:D12')->applyFromArray($style_merge_wrap);
	   
	   //cek tinggi baris
	   $lineBreaks = substr_count($partner_address, "\n") + 1; 
       $estimatedHeight = $lineBreaks * 15;
	   $phpExcel->getActiveSheet()->mergeCells('A13:D13');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("A13",$partner_address, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('A13:D13')->applyFromArray($style_merge_wrap);
	   $phpExcel->getActiveSheet()->getRowDimension(13)->setRowHeight(34.50);
	   
	   $phpExcel->getActiveSheet()->mergeCells('A14:D14');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("A14",$partner_city, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('A14:D14')->applyFromArray($style_merge_wrap);
	   
	   $phpExcel->getActiveSheet()->mergeCells('A15:D15');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("A15",$partner_state, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('A15:D15')->applyFromArray($style_merge_wrap);
	   ...
	   $phpExcel->getActiveSheet()->mergeCells('B18:D18');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B18",$r[0]->item_base_code , PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B18:D18')->applyFromArray($style_merge_wrap);
	   
	   $phpExcel->getActiveSheet()->mergeCells('B19:F19');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B19",$r2[0]->item_base_name, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B19:F19')->applyFromArray($style_merge_wrap);
	   
	   $phpExcel->getActiveSheet()->mergeCells('B20:F20');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B20",$fabric_content, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B20:F20')->applyFromArray($style_merge_wrap);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B21",$weight, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B21')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B22",$width, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B22')->applyFromArray($style_font);
	  
	   $phpExcel->getActiveSheet()->mergeCells('B23:C23');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B23",'w='.$w .' L='.$l.' T='.$t, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B23:C23')->applyFromArray($style_merge_wrap);
	   
	   $phpExcel->getActiveSheet()->mergeCells('B24:C24');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B24",$packing_instructions, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B24:C24')->applyFromArray($style_merge_wrap);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B25",$shipping_sample, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B25')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B26",$requested_etd, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B26')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B27",$date_etd, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B27')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("D27",$date_eta, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('D27')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->setCellValueExplicit("F27",$date_in, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('F27')->applyFromArray($style_font);
	   
	   $phpExcel->getActiveSheet()->mergeCells('B28:G30');
	   $phpExcel->getActiveSheet()->setCellValueExplicit("B28",$other_instructions, PHPExcel_Cell_DataType::TYPE_STRING);
	   $phpExcel->getActiveSheet()->getStyle('B28:G30')->applyFromArray($style_merge_wrap);
		  ...
		  foreach($q->result() as $r){
			...
			$phpExcel->getActiveSheet()->setCellValueExplicit('A'.$i,$r->colour, PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('A'.$i)->applyFromArray($styleArray1);
			
			$phpExcel->getActiveSheet()->setCellValueExplicit('B'.$i,$r->color_code, PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('B'.$i)->applyFromArray($styleArray1);
			
			$phpExcel->getActiveSheet()->setCellValue('C'.$i,number_format($r->quantity_ordered,0) , PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('C'.$i)->applyFromArray($styleArray1);
			$phpExcel->getActiveSheet()->getStyle('C'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			
			$phpExcel->getActiveSheet()->setCellValue('D'.$i,$qty, PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('D'.$i)->applyFromArray($styleArray1);
			$phpExcel->getActiveSheet()->getStyle('D'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			
			$phpExcel->getActiveSheet()->setCellValue('E'.$i,$amount, PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('E'.$i)->applyFromArray($styleArray1);
			$phpExcel->getActiveSheet()->getStyle('E'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			
			$phpExcel->getActiveSheet()->setCellValueExplicit('F'.$i,$r->purchase_order_detail_memo, PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('F'.$i)->applyFromArray($styleArray1);
			
			$phpExcel->getActiveSheet()->setCellValue('G'.$i,$r->item_code, PHPExcel_Cell_DataType::TYPE_STRING);
	        $phpExcel->getActiveSheet()->getStyle('G'.$i)->applyFromArray($styleArray1);
		   ...
		   $style_merge_wrap2;
		   $styleArray3;
		   $terbilang=$say.$this->mainconfig->terbilang_English($jml_amount);
		   
		   $phpExcel->getActiveSheet()->setCellValue('A'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('A'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('B'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('B'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('C'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('C'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('C'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('C'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('D'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('D'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('E'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('E'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('F'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('F'.$i)->applyFromArray($style_merge_wrap2);
		   $phpExcel->getActiveSheet()->setCellValue('G'.$i,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('G'.$i)->applyFromArray($style_merge_wrap2);
		   
		   $a=$i+1;
		   $phpExcel->getActiveSheet()->mergeCells('A'.$a.':B'.$a);
	       $phpExcel->getActiveSheet()->setCellValue('A'.$a,'TOTAL', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('A'.$a.':B'.$a)->applyFromArray($style_merge_wrap3);
		   
		   $phpExcel->getActiveSheet()->setCellValue('C'.$a,number_format($jml,0), PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('C'.$a)->applyFromArray($style_merge_wrap3);
		   
		   $phpExcel->getActiveSheet()->setCellValue('D'.$a,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('D'.$a)->applyFromArray($style_merge_wrap3);
		   
		   $phpExcel->getActiveSheet()->setCellValue('E'.$a,$total_amount, PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('E'.$a)->applyFromArray($style_merge_wrap3);
		   
		   $phpExcel->getActiveSheet()->mergeCells('F'.$a.':G'.$a);
	       $phpExcel->getActiveSheet()->setCellValue('F'.$a,'', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('F'.$a.':G'.$a)->applyFromArray($style_merge_wrap3);
		   
		   $a1=$a+1;
		   $phpExcel->getActiveSheet()->mergeCells('A'.$a1.':G'.$a1);
	       $phpExcel->getActiveSheet()->setCellValue('A'.$a1,$terbilang, PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('A'.$a1.':G'.$a1)->applyFromArray($style_merge_wrap3);
		   
		   $b=$a1+2;
		   $phpExcel->getActiveSheet()->mergeCells('A'.$b.':B'.$b);
		   $phpExcel->getActiveSheet()->setCellValueExplicit("A".$b,'Ordered By :', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('A'.$b.':B'.$b)->applyFromArray($stylebold);
		   $phpExcel->getActiveSheet()->getStyle('A'.$b)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		   
		   $phpExcel->getActiveSheet()->mergeCells('E'.$b.':F'.$b);
		   $phpExcel->getActiveSheet()->setCellValueExplicit("E".$b,'Accepted By :', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('E'.$b.':F'.$b)->applyFromArray($stylebold);
		   $phpExcel->getActiveSheet()->getStyle('E'.$b)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		   
		   $c=$b+8 ; 
		   $phpExcel->getActiveSheet()->mergeCells('A'.$c.':B'.$c);
		   $phpExcel->getActiveSheet()->setCellValueExplicit("A".$c,$partner_name, PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('A'.$c.':B'.$c)->applyFromArray($stylebold);
		   $phpExcel->getActiveSheet()->getStyle('A'.$c)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		   
		   $phpExcel->getActiveSheet()->mergeCells('E'.$c.':F'.$c);
		   $phpExcel->getActiveSheet()->setCellValueExplicit("E".$c,'PT.POPSTAR', PHPExcel_Cell_DataType::TYPE_STRING);
	       $phpExcel->getActiveSheet()->getStyle('E'.$c.':F'.$c)->applyFromArray($stylebold);
		   $phpExcel->getActiveSheet()->getStyle('E'.$c)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	   
	   $report_time=date('_Ymd_His'); 
		
		$filename =  "Fabric_purchase_order " . $report_time;
