...
Class fabric_shipment extends CI_Controller { 
   ...
   { 
	...
	function index() {
		...
		$nav_button[] = array('method_id' => 7811441,'title' => 'Preview', 'btn'=>'btn-warning','icon' => 'fa fa-print', 'load' => 'fabric_shipment/function_print_warehouse');
  ...
  if($table_fabric[0]['fabric_shipment_status_id']==0){
                   ...
                   if ($worksheet) {
			        ...
			        // {
			           ...
			           $highestColumn = $worksheet->getHighestColumn();	
                         ...
                         // PEMBACAAN DIMULAI DARI BARIS KE 4
						  for($row=4; $row<=$highestRow; $row++){
							...
							if (strpos($po_number, '#') !== false) {
                                   // 2. Split data berdasarkan simbol #
                                     $hasilSplit = explode('#', $po_number);
								    $po_number = trim($hasilSplit[0]); 
                            }	
						...
						//$plh_field = ['item_id', 'item_code','item_base_id','item_base_code'];
						$plh_field = ['item_id', 'item_code',];
						$db_cek_item = $this->main->cari_data_utuh('item_name', $colour, 'view_purchase_order_detail_header',
						'purchase_order_no',$po_number, $plh_field);
					
						       foreach ($db_cek_item as $itm) {
							        $item_id=$itm['item_code'];						 
						        }
								
								if ($po_number != '') $all_po_numbers[] =trim($po_number);
								if ($item_id != '')  $all_item_ids[] =trim($item_id);
								if ($colour != '')  $all_colour[] =trim($colour);
                   ...
                   if (!empty($all_po_numbers)) {
					 ...
					 // $db_pos = $this->main->getData_pop('dbo.mt_purchase_order_ecc', null, $field_po , null, $order_cek, null, $offset_cek);
                       ...
                       //     $errors[] = "Di Baris : " . $row . " dan Nomor PO <b>".$po_number." </b> tidak ditemukan di database. <br>";
						...
						//	}
			//			$field_colour =array_unique($all_colour);
			//			$field_po_new =array_unique($all_po_numbers);
			//			$pilih_field = ['item_id', 'item_code','item_base_id','item_base_code'];
			//			$cek_item = $this->main->cari_kata_utuh_new('item_name', $field_colour, 'view_purchase_order_detail_header',
			//			'purchase_order_no',$field_po_new, $pilih_field);
			//			
			//			  foreach ($cek_item as $item) {
            //                      // $map_po[$p['purchase_order_no']] = $p;
			//			      	$map_item[$item['item_code']] = $item['item_id'];
			//					$key_item = $item['item_id'] . '_' . $item['item_code'];
            //                    $map_detail_item[$key_item] = $item;
			//					$map_item_base[$item['item_base_code']] = $item['item_id'];
			//			      	$all_po_item_ids[] = $item['item_id'];
            //                   }
				     }
					 
					//var_dump($map_item);die();
				    ...
				    if (!empty($all_po_ids) && !empty($map_item)) {
						// Ambil semua item_id yang unik dari map_item  map_item_po
			        ...
			        // {
					   ...
					   for($row=4; $row<=$highestRow; $row++){
						   ...
						   $qty_unit =$worksheet->getCellByColumnAndRow(10, $row)->getValue();
						    ...
						    //------ Ini untuk cek jumlah PO yang ada 
							      //--------- JIKA PO ada 2 maka error 
							  //--- Cek data PO di excel jika tidak ada simbol # maka akan dicek jika ada akan dilewat 
							if (strpos($po_number, '#') !== false) {
                                   // 2. Split data berdasarkan simbol #
                                   // Hasilnya berupa array: ["po_number", "OK"]
                                   $hasilSplit = explode('#', $po_number);
								   $po_number = trim($hasilSplit[0]); 
							        //echo "Simbol ditemukan! ID adalah: " . $orderID;
                            } else {
								 $po_cek = $po_number . '%';
						         $db_cek_item_dtl_cek = $this->main->cari_data_utuh_cek('item_name', $colour, 'view_purchase_order_detail_header',
						         'purchase_order_no',$po_cek, $plh_field);
								
								 // echo "Simbol # tidak ditemukan." dan lakukan pengecekan kembali
                      	      if (count($db_cek_item_dtl_cek) > 1) {
                               	//  $errors[] = "<b>Baris " . $row . "</b><br> Nomor PO ".$po_number." <br> di lebih dari 1. <br>";
								 $errors[] = "<b>Baris " . $row . "</b><br>Ditemukan PO ".$po_number." yang hampir sama di Purchase Order ECC <br> 
								edit no PO yg sesuai dok BC di excel <br>
								contoh :(nomor PO # OK)<br>";
							    continue; // <--- Berhenti total, script keluar dari loop for
                               }
							  
							} // akhir cek simbol #
						
						   // karena salah item code maka menggunakan proses yang lain
						   $plh_field = ['item_id', 'item_code',];
						   $db_cek_item_dtl = $this->main->cari_data_utuh('item_name', $colour, 'view_purchase_order_detail_header',
						   'purchase_order_no',$po_number, $plh_field);
						   
						  
						  
						   foreach ($db_cek_item_dtl as $itm_d) {
							   $item_code=$itm_d['item_code'];						 
						   }
						  ...
						  // var_dump($po_number);die();
						   if (empty(trim($po_number))) {
						    ...
						    //pasang pengecekan jika kode barcode kosong 
						   if (empty($barcode_number)){
							   $errors[] = "<b>Baris " . $row . ":</b> Nomor PO ".$po_number." tidak ditemukan kode barcode. <br>";
							   continue;
						   }
							...
							if(!isset($map_item[trim($item_code)])) {
                                    $errors[] = "<b>Baris " . $row . ":</b> item code ".$item_code." (".$colour.") tidak ditemukan.<br>";
									 continue;
                            } 
   ...
   public function import_excel_batch($file_path) {
	...
	function cetak_shipment(){
	   $this->db_pop = $this->load->database('pop',TRUE);
	   $fabric_shipment_id = (isset($_GET['fabric_shipment_id']) && !empty($_GET['fabric_shipment_id']))?$_GET['fabric_shipment_id']:die('{"sts":"ERROR","desc":" Param Header Tidak Ditemukan"}');
	   $data['fabric_shipment_id'] = $fabric_shipment_id;
	   $this->load->view('draft/warehouse/cetak_shipment',$data);
	}
	...
	function download_shipment(){ 
		...
		$phpExcel = PHPExcel_IOFactory::load(FCPATH."/assets/template/warehouse/detail_packing_list_shipment.xlsx");	
		$phpExcel->setActiveSheetIndex(0);  //set first sheet as active
		
		$this->db_pop = $this->load->database('pop', TRUE);
		
		$fabric_shipment_id = isset($_REQUEST['fabric_shipment_id']) ? $_REQUEST['fabric_shipment_id'] : 0;
	...
	$style_font = array(
	 ...
	 $this->load->model('main');
     $q = $this->db_pop->query('SELECT a.fabric_shipment_id,b.fabric_shipment_detail_id,a.fabric_shipment_no,a.fabric_shipment_date,a.fabric_shipment_note
	,b.purchase_order_no,b.item_code,b.fabric_shipment_detail_invoice_number,b.quantity,d.item_base_code,b.weight,d.item_base_name,aa.colour,aa.jml_roll FROM dbo.dt_fabric_shipment a left join dbo.view_fabric_shipment_detail b ON a.fabric_shipment_id = b.fabric_shipment_id left join public.dt_mst_item c ON b.item_id = c.item_id left join public.dt_mst_item_base d ON c.item_base_id = d.item_base_id 	left JOIN ( select fabric_shipment_detail_id,fabric_shipment_list_colour as colour,count(fabric_shipment_list_roll) as jml_roll from dbo.dt_fabric_shipment_list 
  group by fabric_shipment_detail_id,fabric_shipment_list_colour ) aa ON b.fabric_shipment_detail_id = aa.fabric_shipment_detail_id WHERE a.fabric_shipment_id='. $fabric_shipment_id .'order by b.fabric_shipment_detail_id ASC ');
   // $r =$q->result();
	//var_dump($q->result());die();	
	 $rh=3;
	 // $numrow = 17; //	
     foreach($q->result() as $r){
		  // $r->fabric_shipment_detail_invoice_number
		  $phpExcel->getActiveSheet()->setCellValueExplicit("A".$rh,'I/V NO. : '.$r->fabric_shipment_detail_invoice_number, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->applyFromArray($stylebold);	
		  $rh=$rh+1; //4
		  $phpExcel->getActiveSheet()->setCellValueExplicit("D".$rh,'Date : '.$r->fabric_shipment_date, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->applyFromArray($style_font);
		  $phpExcel->getActiveSheet()->setCellValueExplicit("A".$rh,'DESCRIPTION:', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->applyFromArray($stylebold);
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh, $r->item_base_code, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //5
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh, $r->item_base_name, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+2; //7
		  $phpExcel->getActiveSheet()->setCellValueExplicit("D".$rh,$r->fabric_shipment_no, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->mergeCells('D'.$rh.':E'.$rh);
		  $phpExcel->getActiveSheet()->getStyle('D'.$rh.':E'.$rh)->applyFromArray($styleArray3);	
		  $phpExcel->getActiveSheet()->getStyle('D'.$rh.':E'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $rh=$rh+1; //8
		  
		  $phpExcel->getActiveSheet()->setCellValueExplicit("A".$rh,'SHIPPING MARK:', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->applyFromArray($stylebold);
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'BUYER : PT.POPSTAR ', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //9
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'PO : '.$r->purchase_order_no, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //10
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'ITEM : '.$r->item_code, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //11
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'COLOR :'.$r->colour, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //12
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'QTY (YRD) : '.number_format($r->quantity,4,',','.'), PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //13
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'WEIGHT (KGM) : '.number_format($r->weight,4,',','.'), PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //14
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'ROLL: '.number_format($r->jml_roll,0,',','.'), PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($style_font);
		  $rh=$rh+1; //15
		  $phpExcel->getActiveSheet()->setCellValueExplicit("A".$rh,'I/V NO. : '.$r->fabric_shipment_detail_invoice_number, PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->applyFromArray($stylebold);	
		  
		  $q_dtl = $this->db_pop->query('SELECT a.fabric_shipment_list_colour,a.fabric_shipment_list_lot,a.fabric_shipment_list_bale,a.fabric_shipment_list_roll,a.fabric_shipment_list_weight,a.fabric_shipment_list_quantity FROM dbo.dt_fabric_shipment_list a 
	where a.fabric_shipment_detail_id='. $r->fabric_shipment_detail_id .' order by a.fabric_shipment_detail_id ASC');
	      
		  $rh=$rh+1; //16
		  $phpExcel->getActiveSheet()->setCellValueExplicit("A".$rh,'COLOR', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->applyFromArray($styleHeader);	
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  
		  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh,'LOT', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($styleHeader);	
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  
		  $phpExcel->getActiveSheet()->setCellValueExplicit("C".$rh,'BALE', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('C'.$rh)->applyFromArray($styleHeader);	
		  $phpExcel->getActiveSheet()->getStyle('C'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $phpExcel->getActiveSheet()->getStyle('C'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  
		  $phpExcel->getActiveSheet()->setCellValueExplicit("D".$rh,'ROLL', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->applyFromArray($styleHeader);	
		  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  
		  $phpExcel->getActiveSheet()->setCellValueExplicit("E".$rh,'WEIGHT (KGM)', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('E'.$rh)->applyFromArray($styleHeader);	
		  $phpExcel->getActiveSheet()->getStyle('E'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $phpExcel->getActiveSheet()->getStyle('E'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  
		  $phpExcel->getActiveSheet()->setCellValueExplicit("F".$rh,'QTY (YRD)', PHPExcel_Cell_DataType::TYPE_STRING);
		  $phpExcel->getActiveSheet()->getStyle('F'.$rh)->applyFromArray($styleHeader);	
		  $phpExcel->getActiveSheet()->getStyle('F'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  $phpExcel->getActiveSheet()->getStyle('F'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		  
		   $rh=$rh+1; //17
	      
		   foreach($q_dtl->result() as $dtl){
			  $phpExcel->getActiveSheet()->setCellValueExplicit("A".$rh, $dtl->fabric_shipment_list_colour, PHPExcel_Cell_DataType::TYPE_STRING);
			  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->applyFromArray($styleArray1);	
			  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  $phpExcel->getActiveSheet()->getStyle('A'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  
			  $phpExcel->getActiveSheet()->setCellValueExplicit("B".$rh, $dtl->fabric_shipment_list_lot, PHPExcel_Cell_DataType::TYPE_STRING);
			  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->applyFromArray($styleArray1);	
			  $phpExcel->getActiveSheet()->getStyle('B'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  
			  $phpExcel->getActiveSheet()->setCellValueExplicit("C".$rh, $dtl->fabric_shipment_list_bale, PHPExcel_Cell_DataType::TYPE_STRING);
			  $phpExcel->getActiveSheet()->getStyle('C'.$rh)->applyFromArray($styleArray1);	
			  $phpExcel->getActiveSheet()->getStyle('C'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  $phpExcel->getActiveSheet()->getStyle('C'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  
			  $phpExcel->getActiveSheet()->setCellValueExplicit("D".$rh, $dtl->fabric_shipment_list_roll, PHPExcel_Cell_DataType::TYPE_STRING);
			  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->applyFromArray($styleArray1);	
			  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  $phpExcel->getActiveSheet()->getStyle('D'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  
			  $phpExcel->getActiveSheet()->setCellValueExplicit("E".$rh,number_format($dtl->fabric_shipment_list_weight,4,',','.'), PHPExcel_Cell_DataType::TYPE_STRING);
			  $phpExcel->getActiveSheet()->getStyle('E'.$rh)->applyFromArray($styleArray1);	
			  $phpExcel->getActiveSheet()->getStyle('E'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  $phpExcel->getActiveSheet()->getStyle('E'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  
			  $phpExcel->getActiveSheet()->setCellValueExplicit("F".$rh, number_format($dtl->fabric_shipment_list_quantity,4,',','.'), PHPExcel_Cell_DataType::TYPE_STRING);
			  $phpExcel->getActiveSheet()->getStyle('F'.$rh)->applyFromArray($styleArray1);	
			  $phpExcel->getActiveSheet()->getStyle('F'.$rh)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			  $phpExcel->getActiveSheet()->getStyle('F'.$rh)->getAlignment()->setVertical(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
	    ...
	    $filename = "Detail_packing_list_ " . $report_time;
		...
		$return['namafile'] =$filename; 
	  
	   echo json_encode($return);
	}
	
	
	function download_shipment_1(){ 
	  $this->db_pop = $this->load->database('pop',TRUE);
	  $this->load->model('main');
