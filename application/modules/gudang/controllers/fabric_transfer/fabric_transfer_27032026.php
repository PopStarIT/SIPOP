...
class fabric_transfer extends CI_Controller
{
	...
	{
		//$view = 'view_stock_move_supply_transfer';
		$view = 'view_stock_move_supply_transfer_sipop';
		$get_field = $this->ecc_library->get_field_pop($view);
		...
		$get_field['act']['search'] = false;
		$get_field['act']['sortable'] = false;
		$get_field['act']['formatter'] = 'formatOperations_fabric_supply_transfer';
		$get_field['act']['width'] = 300;
	...
	function manual_supply_item_table()
	{
		//$view = 'view_stock_move_supply_transfer';
		//$get_field = $this->ecc_library->get_field($view);

		$view = 'view_stock_move_manual_transfer';
		$get_field = $this->ecc_library->get_field_pop($view);
		...
		//$get_field['r15']['width'] = 200;

		//$get_field['r11']['title'] = 'Colour';
		//$get_field['r12']['title'] = 'Lot';
		//$get_field['r12']['title'] = 'Lot';
		//$get_field['r13']['title'] = 'Lot';
		//$get_field['r14']['title'] = 'No barcode';

		$get_field['r4']['align'] = 'center';
		$get_field['r5']['align'] = 'center';
		$get_field['r6']['align'] = 'center';

		$get_field['r7']['footer'] = 'Total Available :';
	...
	function index()
	{
		...
		$component['page_title'] = "Fabric Transfer";
		...
		$nav_button[] = array('method_id' => 7811383, 'title' => 'Approve', 'btn' => 'btn-success', 'icon' => 'fa fa-thumbs-up', 'load' => 'fabric_transfer/function_approve');
		$nav_button[] = array('method_id' => 7811384, 'title' => 'Cancel Approve', 'btn' => 'btn-info', 'icon' => 'fa fa-thumbs-down', 'load' => 'fabric_transfer/function_cancel_approve');
		$nav_button[] = array('method_id' => 7811379, 'title' => 'Delete', 'btn' => 'btn-danger', 'icon' => 'fa fa-trash-o', 'load' => 'fabric_transfer/function_delete');
		//$nav_button[] = array('method_id' => 7811413, 'title' => 'print', 'btn' => 'btn-primary', 'icon' => 'fa fa-print', 'load' => 'fabric_transfer/function_print');
	...
	function loaddata_transfer_supply()
	{
		...
		$fabric_transfer_detail_id = isset($_REQUEST['fabric_transfer_detail_id']) ? is_numeric($_REQUEST['fabric_transfer_detail_id']) ? $_REQUEST['fabric_transfer_detail_id']  : -1 : -1;
		$fabric_transfer_id = isset($_REQUEST['fabric_transfer_id']) ? is_numeric($_REQUEST['fabric_transfer_id']) ? $_REQUEST['fabric_transfer_id']  : -1 : -1;
		$methodid = isset($_REQUEST['methodid']) ? is_numeric($_REQUEST['methodid']) ? $_REQUEST['methodid']  : -1 : -1;
		//var_dump('ok');die();
		$view = 'view_stock_move_supply_transfer_sipop';
		...
		$extra_param['where']['0']['data'] = $fabric_transfer_detail_id;
	...
	function loaddata_manual_supply()
	{
		$this->authentication->plainlayout();
		...
		//$view = 'view_stock_move_supply_transfer';
		$view = 'view_stock_move_manual_transfer';
		$field = $this->manual_supply_item_table();
		...
		$extra_param['sp_sum'] = 'dbo.sp_fabric_transfer_supply_sum';

		//$loaddata = $this->ecc_library->get_field_data($view,$field,$extra_param);
		//$loaddata = $this->ecc_library->get_field_data_pop_transfer_item($view,$field,$extra_param);
	...
	function approve()
	{
		...
		$fabric_transfer_id = isset($_POST['fabric_transfer_id']) ? $_POST['fabric_transfer_id'] : false;
		...
		if (count($_POST) > 0) {
			...
			if ($fabric_transfer_id) {
				$this->rpc_service->setSP("dbo.sp_fabric_transfer_approve");
	...
	function cancel_approve()
	{
		...
		if (count($_POST) > 0) {
			...
			if ($fabric_transfer_id) {
				$this->rpc_service->setSP("dbo.sp_fabric_transfer_cancel_approve");
				$this->rpc_service->addField('fabric_transfer_id', $fabric_transfer_id);
			}
	...
	function delete()
	{
		...
		if (count($_POST) > 0) {
			...
			if ($fabric_transfer_id) {
				$this->rpc_service->setSP("dbo.sp_fabric_transfer_delete");
				$this->rpc_service->addField('fabric_transfer_id', $fabric_transfer_id);
			}
	...
	function delete_supply_transfer()
	{
		$this->authentication->plainlayout();
		$parameter = array();
		$return = array();

		$fabric_transfer_supply_id = isset($_POST['fabric_transfer_supply_id']) ? $_POST['fabric_transfer_supply_id'] : false;
		$user_id = $this->session->userdata('user_id');
		...
		if (count($_POST) > 0) {
			if ($fabric_transfer_supply_id) {
				$this->rpc_service->setSP("dbo.sp_fabric_transfer_supply_delete");
				...
				if (isset($result['valid'])) {
					if ($result['valid']) {
						if (isset($result['data'])) {
							...
							$return['status_code'] = $result['no'];
							$return['message'] = $result['des'];
							$return['fabric_transfer_supply_id'] = $data_result['fabric_transfer_supply_id'];
						}
					} else {
						$return['status_code'] = $result['no'];
						$return['message'] = $result['des'];
				...
				if (isset($result['valid'])) {
					...
					if ($result['valid']) {
						if (isset($result['data'])) {
							...
							$return['item_code'] = $data_result['item_code'];
	...
	function print_fabric_transfer()
	{
		//7811413
		$this->db_pop = $this->load->database('pop', TRUE);
		$fabric_transfer_supply_id = (isset($_GET['fabric_transfer_supply_id']) && !empty($_GET['fabric_transfer_supply_id'])) ? $_GET['fabric_transfer_supply_id'] : die('{"sts":"ERROR","desc":" Param Header Tidak Ditemukan"}');
		$data['fabric_transfer_supply_id'] = $fabric_transfer_supply_id;

		// $this->load->view('draft/warehouse/draft_barcode',$data);
		$this->load->view('draft/warehouse/draft_cetak_fabric_transfer', $data);
	}
}
