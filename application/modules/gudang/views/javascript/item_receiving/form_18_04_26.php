...
//       }).trigger('resize');
	...
	function save_receive_<?php echo $methodid ?>() {
      ...
      //  $('#chk_add_directly').prop('checked', true);
	 //   scan_barcode_<?php echo $methodid ?>(kode);
	}
   ...
   $('#form_<?php echo $methodid ?>_mdl_add_receive_mdl_shipment_code').on('select2:select', function (e) { 
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.status == 'continue') {
						  ...
						  //  $('#status_pesan').text("Selesai! Semua data telah masuk.");
	...
	function add_all_receive_<?php echo $methodid ?>() {
		...
		if (fabric_shipment_id == null || fabric_shipment_id == '' ){
			show_error("show", 'Error', "Data Shipment kosong");
		}else{
			...
			swal({
			    ...
			    allowOutsideClick : false,
				       ...
				       if (result.value) {
					        ...
					        $.ajax({
					        	url: baseurl+'<?php echo $class_uri ?>/receive_allxx',								
					        	data: {'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>',fabric_receive_id:row.r1},
					        	dataType: 'json',
					        	method: 'post',
					        	success: function(data){
							          page_loading("hide");
