...
//       }).trigger('resize');
	...
	$('#form_<?php echo $methodid ?>_detail_scan_fabric_barcodexxx').keydown(function(e) {
	   ...
	   if (e.key === 'Enter') {
		   $('#lot_quantity_<?php echo $methodid ?>').text(0);
		   $('#lot_number_<?php echo $methodid ?>').text(0);
	       $('#po_number_<?php echo $methodid ?>').text(0);
		   $('#form_<?php echo $methodid ?>_detail_scan_kode_barcode').val(fabric_barcode)
   ...
   function modal_report_receive_<?php echo $methodid ?>(){
	   $('#form_<?php echo $methodid ?>_detail_scan_fabric_warehouse_receive_id').val();
	
	  $('#modal_report_receive').modal('show');
	      
	    setTimeout(() => {
           $("#table_<?php echo $methodid ?>_mdl_rpt_receive").trigger('reloadGrid');
	       $("#table_<?php echo $methodid ?>_mdl_rpt_receive").setGridWidth($('.grid_container_<?php echo $methodid; ?>_mdl_rpt_receive').width() - 20, true).trigger('resize');
        }, 2000); 
	}
	
	function save_receive_<?php echo $methodid ?>() {
		page_loading("show", '<?php echo $page_title ?>', 'Please Wait...');
	    var fabric_warehouse_receive_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_warehouse_receive_id').val();
		var fabric_shipment_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_shipment_id').val();
		var fabric_receive_detail_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_receive_detail_id').val();
		var fabric_barcode = $('#form_<?php echo $methodid ?>_detail_scan_kode_barcode').val();
		var note = $('#form_<?php echo $methodid ?>_detail_scan_note').val();
		var no_kontainer= $('#form_<?php echo $methodid ?>_detail_scan_frm_nomor_kontainer').val();
		//var kode_kontainer = $('#kode_kontainer').val();
		$.ajax({
			url: baseurl + '<?php echo $class_uri ?>/post_add_edit_scan',
			data: {
				'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
			...
			success: function(data) {
				 page_loading("hide");
				 $('.form_<?php echo $methodid ?>_save_receive').hide();
	...
	function scan_barcode_yg_lain<?php echo $methodid ?>(code_barcode) {
		//page_loading("show", '<?php echo $page_title ?>', 'Please Wait...');
		var fabric_warehouse_receive_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_warehouse_receive_id').val();
		var fabric_shipment_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_shipment_id').val();
		var fabric_receive_detail_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_receive_detail_id').val();
		...
		$.ajax({
			...
			success: function(data) {
				...
				// page_loading("hide");
				if (typeof data.fabric_shipment_list_lot === "undefined") {
					 cek = 0 ;
				...
				if ( cek === 1 ){  // Jika langsung
                 ...
                 if (data.valid) {
				  ...
				  //if(strlen(code_barcode) >7){
				  if (data.total_lot != '') {
					  $('#lot_quantity_<?php echo $methodid ?>').text(data.quantity);
					  $('#lot_number_<?php echo $methodid ?>').text(data.fabric_shipment_list_lot);
					  $('#po_number_<?php echo $methodid ?>').text(data.purchase_order_no);
					  //$('#form_<?php echo $methodid ?>_detail_scan_total_lot').val(data.total_lot);
					  //$('#form_<?php echo $methodid ?>_detail_scan_total_roll').val(data.total_roll);
				...
				}else if ( cek = 3 ) { // Jika tidak langsung
			          $('.form_<?php echo $methodid ?>_save_receive').show();
				   ...
				   show_error("show", 'Error', 'Data masih kosong!');	
				    $('.form_<?php echo $methodid ?>_save_receive').hide();
   ...
   function scan_barcode_<?php echo $methodid ?>(code_barcode) {
		...
		//var kode_kontainer = $('#kode_kontainer').val();
		var no_kontainer= $('#form_<?php echo $methodid ?>_detail_scan_frm_nomor_kontainer').val();
		var limit = $('#form_<?php echo $methodid ?>_detail_scan_limit').val(5);
	    const checkbox = document.getElementById('chk_add_directly');
        
		//alert (kode_kontainer);
          // Cek saat ini juga
          if (checkbox.checked) {
            //  console.log("Checkbox dicentang!");
			var cek = 1 ; 
		...
		$.ajax({
			...
			success: function(data) {
                  ...
                  // Hentikan suara lain yang mungkin sedang berjalan (reset)
                  $('audio').each(function(){
                      this.pause();
                      this.currentTime = 0;
                  });
			...
			if ( cek === 1 ){  // Jika langsung
				  ...
				  //$("label[id=label_message]").text(data.message);
				   if (data.total_lot != '') {
					  ...
					  $('#lot_number_<?php echo $methodid ?>').text(data.fabric_shipment_list_lot);
					  $('#po_number_<?php echo $methodid ?>').text(data.purchase_order_no);
					  let isitotal= data.total_diterima + ' to ' + data.total_roll;
					  $('#po_total_<?php echo $methodid ?>').text(isitotal);
					  //$('#form_<?php echo $methodid ?>_detail_scan_total_lot').val(data.total_lot);
					  //$('#form_<?php echo $methodid ?>_detail_scan_total_roll').val(data.total_roll);
					  $('#form_<?php echo $methodid ?>_detail_scan_fabric_shipment_list_lot').val(data.fabric_shipment_list_lot);
					  
					//   $('#total_diterima').text(data.total_item);
					
				  }
				  
				     $('#form_<?php echo $methodid ?>_detail_scan_fabric_barcode').focus();
				     $('#form_<?php echo $methodid ?>_detail_scan_fabric_barcode').val('');
					 $('#form_<?php echo $methodid ?>_detail_scan_note').val('');
				  
		         // showNotification(data.valid)  //Efek suara di komentar dahulu jika menggunakan sound tanpa function
         ...
         if (data.valid) {
				...
				swal({
                 ...
                 // content: wrapper,				  
				  html:html+'<input type="text" id="qty_supply" class="swal2-input" name="qty_supply" value='+data.quantity+'></br><label for="nama" style="display: block; text-align: center;">'+ data.message +'</label>',
				//  input: 'text', // Tipe input (akan tersembunyi karena kita pakai 'html' kustom, tapi tetap berguna untuk fungsionalitas janji/promise)
				  ...
				  focusConfirm: false, // Agar fokus ke input pertama
				    preConfirm: () => {
					    var data_send={
						   ...
						   $.ajax({
                             ...
                             success: function(msg){
								    ...
								    // showNotification(data.valid)  //Efek suara
                        ...
                        /* Logic untuk Cancel:
                           Cek apakah modal ditutup karena tombol cancel 
                        */
						if (!result.isConfirmed) {
                             // Jalankan fungsi hapus teks
                             $('#form_<?php echo $methodid ?>_detail_scan_fabric_barcode').focus();
				             $('#form_<?php echo $methodid ?>_detail_scan_fabric_barcode').val('');
                              //console.log("Input dibersihkan karena user membatalkan.");
	...
	function showNotification(status) {
          ...
          if (sound) {
              ...
              sound.play().catch(function(error) {
                  console.log("Suara diblokir browser sebelum ada interaksi user.");
              });
          }
          
          // 2. Jalankan Getar (Mobile)
	 ...
	 function scan_barcod_OK_14042026<?php echo $methodid ?>(code_barcode) {
		//page_loading("show", '<?php echo $page_title ?>', 'Please Wait...');
		var fabric_warehouse_receive_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_warehouse_receive_id').val();
		var fabric_shipment_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_shipment_id').val();
		var fabric_receive_detail_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_receive_detail_id').val();
				  ...
				  focusConfirm: false, // Agar fokus ke input pertama
				    preConfirm: () => {
                        ...
                        /* Logic untuk Cancel:
                           Cek apakah modal ditutup karena tombol cancel 
                        */
						if (result.dismiss === swal.DismissReason.cancel) { // untuk jika keluar hanya klick tombol cancel
						//if (result.didDismiss) {  // untuk jika keluar dengan klick sembarangan
						//if (result.isDismissed) {
                         	console.log("Tombol cancel ditekan!");
							$('#form_<?php echo $methodid ?>_detail_scan_fabric_barcode').focus();
				            $('#form_<?php echo $methodid ?>_detail_scan_fabric_barcode').val('');
                          // Tambahkan fungsi hapus data input di sini
                         // hapusInputTeks();
   ...
   function receive_list_<?php echo $methodid ?>(receive_id) {
		...
		$.ajax({
			...
			data: {
				...
				fabric_warehouse_receive_detail_id:receive_id,
				code_barcode: code_barcode,
	...
	function receive_list_old_<?php echo $methodid ?>(receive_id) {
   ...
   $('#form_<?php echo $methodid ?>_mdl_add_receive_mdl_shipment_code').on('select2:select', function (e) { 
	...
	$('#form_<?php echo $methodid ?>_detail_item_id,#form_<?php echo $methodid ?>_partner_id,#form_<?php echo $methodid ?>_currencies_id,#form_<?php echo $methodid ?>_purchase_order_date ').on('change', function(event, clickedIndex, newValue, oldValue) {
	 ...
	 $('#form_<?php echo $methodid ?>_detail_scan_no_kontainer1').on('select2:select', function (e) { 
	   var datakontainer_db = e.params.data;
	  // console.log(datakontainer_db);
	   $('#form_<?php echo $methodid ?>_detail_scan_frm_nomor_kontainer').val(datakontainer_db.id)
	   
	   if (datakontainer_db != '' && datakontainer_db != 0 ){
		  $("#table_<?php echo $methodid ?>_receive_scan").trigger('reloadGrid');
	      $("#table_<?php echo $methodid ?>_receive_scan").setGridWidth($('.grid_container_<?php echo $methodid; ?>_receive_scan').width() - 20, false).trigger('resize');
		  
		                      //        // Cek apakah checkbox dicentang (true/false)
                              //           var isChecked = $('#check_limit_all').is(':checked');
                              //           
                              //           // Reload jqGrid dengan parameter tambahan
                              //           $("#table_<?php //echo $methodid ?>_receive_scan").jqGrid('setGridParam', {
                              //               postData: {
                              //                   // Kita kirim parameter custom 'use_limit' ke PHP
                              //                   use_limit: isChecked ? 1 : 0 
                              //               },
                              //               page: 1
                              //           }).trigger("reloadGrid");
							  //
                              //           // Opsional: Sembunyikan navigasi halaman secara visual jika limit aktif
                              //           if (isChecked) {
                              //               $("#table_<?php //echo $methodid ?>_receive_scan").jqGrid('setGridParam', { 
                              //           //        rowNum: 1000, // Nilai besar untuk "semua data"
                              //                   pgbuttons: true, 
                              //                   pginput: true 
								//				   
                              //               });
		   ...
		   show_error("show", 'Error', 'Pilih Data Kontainer');
	   }
	 ...
	 $('#kode_kontainer').on('change', function() {
		 var nilaikontainer = $(this).val();
		// const selected = $(this).find('option:selected');
		 $('#form_<?php echo $methodid ?>_detail_scan_frm_nomor_kontainer').val(nilaikontainer)
		 
		  if (nilaikontainer != 'AZ' ){
		  $("#table_<?php echo $methodid ?>_receive_scan").trigger('reloadGrid');
	      $("#table_<?php echo $methodid ?>_receive_scan").setGridWidth($('.grid_container_<?php echo $methodid; ?>_receive_scan').width() - 20, false).trigger('resize');
    ...
    function add_kontainer_<?php echo $methodid ?>() {
		$('#form_<?php echo $methodid ?>_detail_scan_no_kontainer1').html('');
		$('#form_<?php echo $methodid ?>_detail_scan_no_kontainer1').selectpicker('refresh');
		change_form_<?php echo $methodid ?>_detail_scan_no_kontainer1(0);
		$('#form_<?php echo $methodid ?>_detail_scan_frm_nomor_kontainer').val(0);
		
				
	//$("#table_<?php echo $methodid ?>_receive_scan").trigger('reloadGrid');
	 //   $("#table_<?php echo $methodid ?>_receive_scan").setGridWidth($('.grid_container_<?php echo $methodid; ?>_receive_scan').width() - 20, false).trigger('resize');
		
		$('#plh_kontainer_db').hide();
		$('#plh_kontainer').show();
	...
	function edit_<?php echo $methodid ?>(id) {
		...
		$.ajax({
			...
			success: function(data) {
                ...
                //$('#plh_kontainer').html('');
		        //$('#plh_kontainer').selectpicker('refresh');
				//$('#plh_kontainer').prop('selectedIndex', 0);
				$('#form_<?php echo $methodid ?>_detail_scan_frm_nomor_kontainer').val(0);
				$('#plh_kontainer').val('AZ');
                $('#plh_kontainer_db').show();
		        $('#plh_kontainer').hide();
	...
	function post_<?php echo $methodid ?>() {
		if (check_submit == 0) {
			...
			$('#status_<?php echo $methodid ?>_container').show();
			$('#status_pesan').text("Sedang memproses, mohon tunggu...");
			//console.log(data);
			$.ajax({
				...
				success: function(data) {
					...
					if (data.status == 'continue') {
                          // console.log(data.message);
						   $('#status_pesan').text("Berhasil memproses " + data.message);
                              // PANGGIL LAGI DIRINYA SENDIRI (RECURSIVE)
							  
                            // jalankanProses(); 
							 post_<?php echo $methodid ?>() ; 
                       } else if (data.status == 'done') {
						   show_success("show", '<?php echo $page_title ?>', 'success' + data.message);
						   $('#status_<?php echo $methodid ?>_container').hide();
						  // $('#loader_icon').html('<i class="fa fa-check text-success"></i>');
                          // $('#status_pesan').text(data.message);
                         //  $('.btn').prop('disabled', false);
						   check_submit = 1;
                         //  alert("Semua data berhasil disinkronisasi!");
												 
                        } else {
							 // 4. Jika terjadi error dari database (sts_no != 0000)
                           $('#loader_icon').html('<i class="fa fa-times text-danger"></i>');
                           $('#status_pesan').text("Gagal: " + data.message);
                           $('.btn').prop('disabled', false);
						   check_submit = 1;
                           alert("Terhenti karena error: " + data.message);
	...
	//				}
				...
				error: function(xhr, error) {
					$('#status_<?php echo $methodid ?>_container').hide();
					show_error("show", xhr.status.toString() + ' ' + xhr.statusText, 'Terjadi kesalahan jaringan atau server');
					 //alert("Please try again: " + xhr.status.toString() + ' ' + xhr.statusText,);
					 //check_submit = 0;
					// $('#status_pesan').text("Terjadi kesalahan jaringan atau server.");
                     $('.btn').prop('disabled', false);
	...
	function add_all_receive_<?php echo $methodid ?>() {
		//var fabric_shipment_id = $('#form_<?php echo $methodid ?>_mdl_add_receive_fabric_shipment_id').val();
		 $('#status_<?php echo $methodid ?>_receive_all').hide();
		var fabric_warehouse_receive_id = $('#form_<?php echo $methodid ?>_detail_scan_fabric_warehouse_receive_id').val();
		if (fabric_warehouse_receive_id == null || fabric_warehouse_receive_id == '' ){
			show_error("show", 'Error', "Data Receive kosong");
		}else{
			//show_error("show", 'Error', "Ada Data");
			swal({
			    title: "Confirm Receive All <?php echo $page_title ?> ?",
			    type: "question",
			    dangerMode: true,
			    showCancelButton: !0,
			    ...
			    allowOutsideClick : false,
				       ...
				       if (result.value) {
					        page_loading("show",'Receive all <?php echo $page_title ?>','Please Wait...');
					        $.ajax({
					        	url: baseurl+'<?php echo $class_uri ?>/post_add_edit_receive_all',								
					        	...
					        	success: function(data){
							          page_loading("hide");
									   $('#status_<?php echo $methodid ?>_receive_all').show();
							          check_submit = 0;
							    if(data.status == 'success'){
									...
									//$('#loader_icon_all').html('<i class="fa fa-hourglass text-info "></i>');
									$('#loader_icon_all').html('<i class="fa fa-check text-success"></i>');
									//$('#loader_icon').html('<i class="fa fa-times text-danger"></i>');
                                    $('#status_pesan_all').text(data.message);
								    //show_success("info",'Delete <?php //echo $page_title ?>',data.message);			
								   // $("#table_<?php //echo $methodid ?>").trigger('reloadGrid');
							      } else {
								    $('#loader_icon_all').html('<i class="fa fa-times text-danger"></i>');
                                    $('#status_pesan_all').text(data.message);
								    show_error("show",'Error',data.message);
							     }
								// $('#status_<?php echo $methodid ?>_receive_all').hide();
