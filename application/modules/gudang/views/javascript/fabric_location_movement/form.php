<script type="text/javascript">
	...
	$(document).ready(function() {
		// Fokuskan kursor ke input barcode saat halaman siap
		$('#form_<?php echo $methodid ?>_fabric_shipment_list_code').focus();

		// Event listener Enter Key
		$('#form_<?php echo $methodid ?>_fabric_shipment_list_code').on('keypress', function(e) {
			if (e.which === 13) {
				e.preventDefault();
		...
		let barcodeTimeout;
		$('#form_<?php echo $methodid ?>_fabric_shipment_list_code').on('input', function() {
			clearTimeout(barcodeTimeout);
			barcodeTimeout = setTimeout(function() {
				let barcodeValue = $('#form_<?php echo $methodid ?>_fabric_shipment_list_code').val();
				if (barcodeValue.length > 0) {
					return_barcode_<?php echo $methodid ?>();
				}
			}, 500); // Tunggu 500ms setelah input terakhir
	...
	function return_barcode_<?php echo $methodid ?>() {
		let input_element = $('#form_<?php echo $methodid ?>_fabric_shipment_list_code');
		let code_barcode = input_element.val().trim();
		let grid_id = "table_<?php echo $methodid ?>";
	...
	function reset_<?php echo $methodid ?>() {
		...
		$('#form_<?php echo $methodid ?>_fabric_shipment_list_code').val('');

		// 2. Tentukan ID grid
		var grid_id = "table_<?php echo $methodid ?>";
