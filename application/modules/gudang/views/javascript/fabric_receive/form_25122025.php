<script type="text/javascript">
	$(".form_tab_<?php echo $methodid ?>").on("click", "a", function(e) {
		...
		setTimeout(function() {
			...
			$("#table_<?php echo $methodid ?>_other_list").setGridWidth($('.grid_container_<?php echo $methodid; ?>_other_list').width() - 20, true).trigger('resize');

			$("#table_<?php echo $methodid ?>_other_receive_scan").trigger('reloadGrid');
			$("#table_<?php echo $methodid ?>_other_receive_scan").setGridWidth($('.grid_container_<?php echo $methodid; ?>_other_receive_scan').width() - 20, true).trigger('resize');
	...
	$('#form_<?php echo $methodid ?>_detail_scan_fabric_barcodex').on('keyup', function(event, clickedIndex, newValue, oldValue) {
	...
	$(function() {
		...
		"use strict";
		$("#table_<?php echo $methodid ?>_other_receive_scan").jqGrid({
			url: baseurl + '<?php echo $class_uri ?>/loaddata_detil_other_scan',
			datatype: "json",
			mtype: "post",
			postData: {
				'q': '1',
				'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
				fabric_warehouse_receive_id: function() {
					return $('#form_<?php echo $methodid ?>_other_scan_fabric_warehouse_receive_id').val();
				}
			...
			colModel: [{
...

				{
					...
					// formatter:'actions',formatoptions:{keys:true}
					formatter: function(cellvalue, options, rowObject) {
						return ' <button class="btn btn-xs btn-success" onclick="javascript:Update_receive_warehouse_other_<?php echo $methodid ?>' + '_other_receive_scan(' + rowObject.r1 + ')"><i class="fa fa-save"></i> Update </button>  <button class="btn btn-xs btn-danger" onclick="javascript:cancel_receive_scan_other_<?php echo $methodid ?>' + '_other_receive_scan(' + rowObject.r1 + ')"><i class="fa fa-ban"></i> Cancel</button>';
						//  return ' <button class="btn btn-xs btn-success" onclick="javascript:Update_receive_warehouse_<?php echo $methodid ?>' + '(' + rowObject.r1 + ')"><i class="fa fa-save"></i> Receive</button> <button class="btn btn-xs btn-info" onclick="javascript:edit_temp_scan_<?php echo $methodid ?>' + '(' + rowObject.r1 + ')"><i class="fa fa-pencil-square-o"></i> Update</button> ';
					}
				}
			],
			...
			rowNum: 30,
			rowList: [10, 20, 30],
			pager: '#ptable_<?php echo $methodid ?>_other_receive_scan',
			sortname: "r1",
			sortorder: "asc",
			shrinkToFit: false,
			//cellEdit: true,
			...
			onSelectRow: function(id) {
				...
				// alert (id +' Dan ' + lastsel2);			
				if (id && id !== lastsel2) {
					jQuery('#table_<?php echo $methodid ?>_other_receive_scan').jqGrid('restoreRow', lastsel2);
					// jQuery('#table_<?php echo $methodid ?>_detail_scan').jqGrid('editRow',id,true);
					lastsel2 = id;
				}
			},
			ondblClickRow: function(id2) {
				...
				if (cek == 1) {
					...
					jQuery('#table_<?php echo $methodid ?>_other_receive_scan').jqGrid('editRow', id2, true);
					cek = 1;
				}
		...
		$("#table_<?php echo $methodid ?>_other_receive_scan").jqGrid("setColProp", "rn", {
			hidedlg: false
		});

		$("#table_<?php echo $methodid ?>_other_receive_scan").jqGrid('navGrid', '#ptable_<?php echo $methodid ?>_other_receive_scan', {
			edit: false,
			add: false,
			del: false,
			view: false,
		...
		$("#table_<?php echo $methodid ?>_other_receive_scan").jqGrid('filterToolbar', {
			stringResult: true,
			searchOnEnter: false,
			defaultSearch: 'cn',
			ignoreCase: false
	...
	function receive_list_<?php echo $methodid ?>(receive_id) {
		...
		let barcode = unwrap_cell_value(row.r14).replace(/,/g, '');
		...
		swal({
			...
			if (result.value) {
				...
				$.ajax({
					...
					data: {
						'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
						fabric_shipment_list_id: shipment_list_id
					},
					dataType: 'json',
					method: 'post',
					success: function(data) {
