<script type="text/javascript">
	...
	function supply_fabric_barcode_<?php echo $methodid ?>(e) {
     ...
     function add_supply_<?php echo $methodid ?>(fabric_transfer_detail_id){
	...
	var click_supply_<?php echo $methodid ?> = function (id, isSelected) {
		...
		if (!isSelected) {	
			...
			fabri_transfer_detail_id = parseInt(unwrap_cell_value(row.r14).replace(/,/g, ''));
			//quantity_supply = parseFloat(unwrap_cell_value(row.r8).replace(/,/g, ''));
			let quantity_supply = parseFloat(unwrap_cell_value(row.r7).replace(/,/g, ''));
