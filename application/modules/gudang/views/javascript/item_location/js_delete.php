	...
	function nav_button_<?php echo $function ?>(){
		...
		if (id) { 
			...
			swal({
				...
				if (result.value) {
					page_loading("show",'Delete Location','Please Wait...');
					$.ajax({
						url: baseurl+'<?php echo $class_uri ?>/delete',								
						data: {'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>',location_id:row.r1},
						dataType: 'json',
						method: 'post',
						success: function(data){
							...
							check_submit = 0;
							if(data.valid){
								show_success("show",'Delete Item',data.message);			
