<script type="text/javascript">
	...
	$(".form_tab_<?php echo $methodid ?>").on("click", "a", function(e) {
		...
		setTimeout(function() {
			...
			$("#table_<?php echo $methodid ?>_detail_trims").trigger('reloadGrid');
			$("#table_<?php echo $methodid ?>_detail_trims").setGridWidth($('.grid_container_<?php echo $methodid; ?>_detail_trims').width() - 20, true).trigger('resize');


			//dari haris
			$("#table_<?php echo $methodid ?>_picture").trigger('reloadGrid');
			$("#table_<?php echo $methodid ?>_picture").setGridWidth($('.grid_container_<?php echo $methodid; ?>_picture').width() - 20, true).trigger('resize');
	...
	$(function() {
			...
			colModel: [{
				...
				},
				{
					...
					formatoptions: {
						onClick: function(cellValue, rowId, rowData, e) {
							...
							var spec_id = row[rowId - 1]['r1'];
							var uraian = row[rowId - 1]['r4'];
							var nilai_info = row[rowId - 1]['r12'];
				...
				},
				{
					...
					formatoptions: {
						onClick: function(cellValue, rowId, rowData, e) {
							...
							var spec_id = row[rowId - 1]['r1'];
							var uraian = row[rowId - 1]['r4'];
							var nilai_info = row[rowId - 1]['r15'];
				...
				},
				{
					...
					formatoptions: {
						onClick: function(cellValue, rowId, rowData, e) {
							...
							var spec_id = row[rowId - 1]['r1'];
							var uraian = row[rowId - 1]['r4'];
							var nilai_info = row[rowId - 1]['r18'];
							//alert(formula_s);
							$('#form_<?php echo $methodid ?>_formula_nama_formula').val('');
							$('#form_<?php echo $methodid ?>_formula_detail_Spec_id').val('');
				...
				},
				{
					...
					formatoptions: {
						onClick: function(cellValue, rowId, rowData, e) {
							...
							var spec_id = row[rowId - 1]['r1'];
							var uraian = row[rowId - 1]['r4'];
							var nilai_info = row[rowId - 1]['r21'];
							//alert(formula_s);
							$('#form_<?php echo $methodid ?>_formula_nama_formula').val('');
							$('#form_<?php echo $methodid ?>_formula_detail_Spec_id').val('');
				...
				},
				{
					...
					formatter: function(cellvalue, options, rowObject) {
						...
						return '<button class="btn btn-xs btn-success" onclick="javascript:simpan_spec_<?php echo $methodid ?>' + '(' + rowObject.r1 + ')"><i class="fa fa-save"></i> Save</button> <button class="btn btn-xs btn-danger" onclick="javascript:delete_spec_<?php echo $methodid ?>_detail_spec' + '(' + rowObject.r1 + ')"><i class="fa fa-trash-o"></i> Delete</button>';
						//delete_spec_
		...
		$("#table_<?php echo $methodid ?>_detail_spec").jqGrid("setColProp", "rn", {
			hidedlg: false
		});

		$("#table_<?php echo $methodid ?>_detail_spec").jqGrid('navGrid', '#ptable_<?php echo $methodid ?>_detail_spec', {
			edit: false,
			add: false,
			del: false,
			view: false,
		...
		$("#table_<?php echo $methodid ?>_detail_spec").jqGrid('filterToolbar', {
			stringResult: true,
			searchOnEnter: false,
			defaultSearch: 'cn',
			ignoreCase: false
	...
	function cancel_trims_<?php echo $methodid ?>_trims() {
		// alert('cancel');
		$('#form_<?php echo $methodid ?>_trims_style_spec_trims_description').val('');
		$('#form_<?php echo $methodid ?>_trims_style_spec_trims_id').val('');
		$('#form_detail_<?php echo $methodid ?>_trims_style_spec_trims_description').val('');
		$('.simpan_trims').show();
		$('.update_trims').hide();
		$('.cancel_trims').hide();
	}
	...
	//dari haris
	function cancel_picture_<?php echo $methodid ?>_picture() {
		// alert('cancel');
		$('#form_picture_<?php echo $methodid ?>_picture_id_image_pattern').val('');
		$('#form_picture_<?php echo $methodid ?>_picture_image_description').val('');
		$('.simpan_picture').show();
		$('.update_picture').hide();
		$('.cancel_picture').hide();
	}
	...
	function cancel_<?php echo $methodid ?>_post_add() {
		// Reset form
		$('#form_<?php echo $methodid ?>_post_add')[0].reset();

		// Hide the new measure group
		$('#new-measure-group').hide();
	...
	function save_<?php echo $methodid ?>_post_add() {
		$('#form_<?php echo $methodid ?>_post_add').submit();
	}


	function save_<?php echo $methodid ?>_trims() {
		$('#form_<?php echo $methodid ?>_trims').submit();
	}


	//dari haris 
	function save_<?php echo $methodid ?>_picture() {

		$('#form_picture_<?php echo $methodid ?>_picture').submit();
	}
	...
	//var_dump(save_<?php echo $methodid ?>_post_add());
	...
	function edit_<?php echo $methodid ?>(id) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#form_<?php echo $methodid ?>_detail_style_spec_header_id').val(data[0].style_spec_header_id);
				$('#form_detail_<?php echo $methodid ?>_trims_style_spec_header_id').val(data[0].style_spec_header_id);
				$('#form_picture_<?php echo $methodid ?>_picture_style_spec_header_id').val(data[0].style_spec_header_id);
	...
	function delete_spec_<?php echo $methodid ?>_detail_spec() {
		var id = jQuery("#table_<?php echo $methodid ?>_detail_spec").jqGrid('getGridParam', 'selrow'); // Corrected ID selector

		if (id) {
			var row = jQuery("#table_<?php echo $methodid ?>_detail_spec").jqGrid('getRowData', id); // Corrected ID selector
			swal({
				...
				if (result.value) {
					...
					$.ajax({
						url: baseurl + '<?php echo $class_uri ?>/delete_spec',
						data: {
							'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
							style_spec_detail_id: row.r1 // Corrected parameter name
						},
						dataType: 'json',
						method: 'post',
						success: function(data) {
							...
							if (data.valid) {
								...
								$("#table_<?php echo $methodid ?>_detail_spec").trigger('reloadGrid'); // Corrected ID selector
							} else {
								show_error("show", 'Error', data.message);
							}
						},
	...
	//dari haris
	function delete_picture_<?php echo $methodid ?>_picture() {
		var id = jQuery("#table_<?php echo $methodid ?>_picture").jqGrid('getGridParam', 'selrow'); // Corrected ID selector

		if (id) {
			var row = jQuery("#table_<?php echo $methodid ?>_picture").jqGrid('getRowData', id); // Corrected ID selector
			swal({
				title: "Confirm Delete <?php echo $page_title ?> ?",
				type: "question",
				dangerMode: true,
				...
				if (result.value) {
					page_loading("show", 'Delete <?php echo $page_title ?>', 'Please Wait...');
					$.ajax({
						url: baseurl + '<?php echo $class_uri ?>/delete_picture',
						data: {
							'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
							id_image_pattern: row.r1 // Corrected parameter name
						},
						dataType: 'json',
						method: 'post',
						success: function(data) {
							...
							if (data.valid) {
								show_success("show", 'Delete <?php echo $page_title ?>', data.message);
								$("#table_<?php echo $methodid ?>_picture").trigger('reloadGrid'); // Corrected ID selector
							} else {
								show_error("show", 'Error', data.message);
							}
						},
	...
	function delete_trims_<?php echo $methodid ?>(style_spec_trims_id) {
		if (check_submit == 0) {
			swal({
				title: "Confirm Delete  Trims ?",
				type: "question",
				dangerMode: true,
				showCancelButton: !0,
				confirmButtonClass: "btn btn-danger m-1",
				...
				if (result.value) {
					//alert(style_spec_trims_id);
					page_loading("show", 'Delete Trims', 'Please Wait...');
					$.ajax({
						url: baseurl + '<?php echo $class_uri ?>/delete_trims',
						data: {
							'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
							trims_id: style_spec_trims_id
						},
						dataType: 'json',
						method: 'post',
						success: function(data) {
							...
							check_submit = 0;
							if (data.valid) {
								show_success("show", 'Delete Trims', data.message);
								//$("#table_<?php echo $methodid ?>_keluarga").trigger('reloadGrid');
								//cancel_keluarga_<?php echo $methodid ?>();
	...
	function post_add_<?php echo $methodid ?>_post_add() {
		if (check_submit == 0) {
			check_submit = 1;
			page_loading("show", '<?php echo $page_title ?>', 'Please Wait...');
			var data = $("#form_<?php echo $methodid ?>_post_add").serialize();
			$.ajax({
				url: baseurl + '<?php echo $class_uri ?>/post_add',
				data: data,
				dataType: 'json',
				method: 'post',
				success: function(data) {
					...
					check_submit = 0;
					if (data.valid) {
						show_success("show", '<?php echo 'point of measure' ?>', data.message, function() {
							location.reload();
						});
						$('#panel_content_<?php echo $methodid ?>_post_add').show();
						$('#panel_content_form_<?php echo $methodid ?>_post_add').hide();
						$('#form_<?php echo $methodid ?>_style_spec_detail_id').val(data.style_spec_detail_id);
					} else {
						show_error("show", 'Error', data.message);
					}
				},
	...
	function edit_trims_<?php echo $methodid ?>(style_spec_trims_id) {
		// alert (style_spec_trims_id);
		$('.simpan_trims').hide();
		$('.update_trims').show();
		$('.cancel_trims').show();
		...
		$.ajax({
			...
			data: {
				'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
				param: 'data_style_spec_trims',
				param_pop: 'db_pop',
				id: style_spec_trims_id
			},
			dataType: 'json',
			method: 'post',
			success: function(data) {
				page_loading("hide");
				//console.log(data[0]);
				$('#form_detail_<?php echo $methodid ?>_trims_style_spec_trims_description').val(data[0].style_spec_trims_description);
				$('#form_detail_<?php echo $methodid ?>_trims_style_spec_trims_id').val(data[0].style_spec_trims_id);
	...
	function edit_picture_<?php echo $methodid ?>_picture(id_image_pattern) {
		$('.simpan_picture').hide();
		$('.update_picture').show();
		$('.cancel_picture').show();
		page_loading("show", '<?php echo $page_title ?>', 'Please Wait...');
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#form_picture_<?php echo $methodid ?>_picture_id_image_pattern').val(data[0].id_image_pattern);
				$('#form_picture_<?php echo $methodid ?>_picture_style_spec_header_id').val(data[0].style_spec_header_id);
				$('#form_picture_<?php echo $methodid ?>_picture_image_description').val(data[0].image_description);
				$('#form_picture_<?php echo $methodid ?>_picture_style_name_image_pattern').val(data[0].name_image_pattern);



				$("#tab_<?php echo $methodid; ?>_picture").attr("data-toggle", "tab");
				$("#tab_<?php echo $methodid; ?>_picture").removeClass("tab_disabled");

				$('.panel_<?php echo $methodid ?>_picture').show();
	...
	function post_edit_<?php echo $methodid ?>_trims() {

		if (check_submit == 0) {
			var data = $("#form_detail_<?php echo $methodid ?>_trims").serialize();
			check_submit = 1;
			page_loading("show", 'Save <?php echo $page_title ?>', 'Please Wait...');
			// console.log(data);
			$.ajax({
				url: baseurl + '<?php echo $class_uri ?>/post_add_edit_trims',
				data: data,
				dataType: 'json',
				method: 'post',
				success: function(data) {
					...
					check_submit = 0;
					if (data.valid) {
						show_success("show", 'Trims Berhasil', data.message);
						if (data.keterangan == 'update') {

							$('#form_detail_<?php echo $methodid ?>_trims_style_spec_trims_id').val('');
							$('.simpan_trims').show();
							$('.update_trims').hide();
							$('.cancel_trims').hide();
						}
	...
	function post_<?php echo $methodid ?>_picture() {
		// alert('coba');
		if (check_submit == 0) {
			check_submit = 1;
			// var_dump('coba');
			page_loading("show", 'Save <?php echo $page_title ?>', 'Please Wait...');
			var data = $("#form_picture_<?php echo $methodid ?>_picture").serialize();

			let frmdata = new FormData();

			var data2 = $("#form_picture_<?php echo $methodid ?>_picture").serializeArray();
			$.each(data2, function(key, input) {
				frmdata.append(input.name, input.value);
			});

			const file_data = $('#form_picture_<?php echo $methodid ?>_name_image_pattern').prop('files')[0];
			// console.log(file_data);
			// console.log(frmdata);
			...
			//Method: 'post',
			$.ajax({
				url: baseurl + '<?php echo $class_uri ?>/post_add_edit_picture',
				type: 'post',
				data: frmdata,
				processData: false,
				contentType: false,
				...
				success: function(data) {
					...
					if (data.valid) {
						...
						if (new_purchase_order == 1) {
							...
							$("#tab_<?php echo $methodid; ?>_picture").attr("data-toggle", "tab");
							$("#tab_<?php echo $methodid; ?>_picture").removeClass("tab_disabled");
							$("#tab_<?php echo $methodid; ?>_picture").click();

							id_image_pattern = data.id_image_pattern;
							$('#form_picture_<?php echo $methodid ?>_picture_id_image_pattern').val(id_image_pattern);
							$('#form_picture_<?php echo $methodid ?>_picture_style_spec_header_id').val(style_spec_header_id);
							$('#form_picture_<?php echo $methodid ?>_picture_image_description').val(image_description);
							$('#form_picture_<?php echo $methodid ?>_picture_name_image_pattern').val(name_image_pattern);
