<script type="text/javascript">
	...
	$(function() {
		...
		$("#table_<?php echo $methodid ?>_detail_spec").jqGrid({
			...
			colNames: ['#', 'DETAIL_ID', 'ID', 'SIZE', 'POINT OF MEASURE', 'BASIC', 'XS', 'PATTERN XS', 'ALLOWANCE XS', 'S', 'PATTERN S', 'ALLOWANCE S', 'M', 'PATTERN M', 'ALLOWANCE M', 'L', 'PATTERN L', 'ALLOWANCE L', 'XL', 'PATTERN XL', 'ALLOWANCE XL', 'XXL', 'PATTERN XXL', 'ALLOWANCE XXL', '#'],
			colModel: [{
					name: 'r25',
					index: 'r25',
					align: 'center',
	...
	function save_<?php echo $methodid ?>_trims() {
		$('#form_detail_<?php echo $methodid ?>_trims').submit();
	}
	...
	function edit_<?php echo $methodid ?>(id) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#form_<?php echo $methodid ?>_picture_1_style_spec_header_id').val(data[0].style_spec_header_id);
				$('#form_<?php echo $methodid ?>_picture_1_style_spec_nomor').val(data[0].nomor);
				
				change_form_<?php echo $methodid ?>_item_id(data[0].item_id);
				
				var img_1 = document.getElementById('gambar1');
				var img_2 = document.getElementById('gambar2');
				//if (data[0].style_spec_header_id != null){
				if(data['image'] != 0){
				     img_1.setAttribute("src", baseurl+'assets/img/pattern/'+data['image'].pic_1);
					 img_2.setAttribute("src", baseurl+'assets/img/pattern/'+data['image'].pic_2);
			     }else{
					//assets/img/pattern/contoh.png
					 img_1.setAttribute("src", baseurl+'assets/img/pattern/sketsa.png');
					 img_2.setAttribute("src", baseurl+'assets/img/pattern/sketsa.png');
				}
		...
		page_loading("show", '<?php echo $page_title ?>', 'Please Wait...');
	  	$.ajax({
	  		...
	  		success: function(data) {
	  			    page_loading("hide");
	  				//console.log(data[0]);
	  			$('#form_<?php echo $methodid ?>_trims_style_spec_trims_description').val(data[0].style_spec_trims_description);
				$('#form_<?php echo $methodid ?>_trims_style_spec_trims_id').val(data[0].style_spec_trims_id);
				...
				if (result.value) {
					...
					$.ajax({
						...
						success: function(data) {
							...
							check_submit = 0;
							if (data.valid) {
								show_success("show", 'Delete <?php echo $page_title ?> Trims', data.message);
								//$("#table_<?php echo $methodid ?>_keluarga").trigger('reloadGrid');
	...
	var check_submit_gbr = 0;
	function add_spec_<?php echo $methodid ?>_gbr_1() {
		if(check_submit_gbr == 0) {
		     check_submit_gbr = 1;
		//	page_loading("show",'Add Image <?php echo $page_title ?>','Please Wait...');
		   	let frmdata= new FormData();
			
			var data2 =  $("#form_<?php echo $methodid ?>_picture_1").serializeArray();
			$.each(data2, function (key,input) {
				frmdata.append(input.name,input.value);
			});
			
			const file_data = $('#form_<?php echo $methodid ?>_lokasi_1').prop('files')[0];
			//console.log(frmdata);
			if(file_data){
	   ...
	   check_submit_gbr2=0
	function add_spec_<?php echo $methodid ?>_gbr_2() {
		if(check_submit_gbr == 0) {
		     check_submit_gbr2 = 1;
		//	page_loading("show",'Add Image <?php echo $page_title ?>','Please Wait...');
		   	let frmdata= new FormData();
			
			var data2 =  $("#form_<?php echo $methodid ?>_picture_2").serializeArray();
			$.each(data2, function (key,input) {
				frmdata.append(input.name,input.value);
			});
			var style_spec_header_id =  $("#form_<?php echo $methodid ?>_picture_1_style_spec_header_id").val();
			var style_spec_nomor =  $("#form_<?php echo $methodid ?>_picture_1_style_spec_nomor").val();
			
			//alert(style_spec_nomor);
			
			const file_data = $('#form_<?php echo $methodid ?>_lokasi_2').prop('files')[0];
			
			if(file_data){
				frmdata.append('file',file_data);
				frmdata.append('style_spec_header_id',style_spec_header_id);
				frmdata.append('style_spec_nomor',style_spec_nomor);
			}
			//console.log(frmdata);
			if(file_data){
				  $.ajax({
	...
	function picture_trims_1(){
		const filegbr= document.getElementById("form_<?php echo $methodid; ?>_lokasi_1");
		let name=filegbr.files[0].name;
		let fjenis=filegbr.files[0].type;
		let fsize=filegbr.files[0].size;
		const ext=fjenis.split("/").pop().toLowerCase();
		...
		//	$("#nmficture").val("");
			return false;
		      ...
		      //alert('asssss4');	
		      const gbr=document.querySelector(".gbrphoto1");
		      filegbr.textContent = name;
	...
	function picture_trims_2(){
		const filegbr= document.getElementById("form_<?php echo $methodid; ?>_lokasi_2");
		let name=filegbr.files[0].name;
		let fjenis=filegbr.files[0].type;
		let fsize=filegbr.files[0].size;
		const ext=fjenis.split("/").pop().toLowerCase();
		...
		//	$("#nmficture").val("");
			return false;
		      ...
		      //alert('asssss4');	
		      const gbr=document.querySelector(".gbrphoto2");
		      filegbr.textContent = name;
