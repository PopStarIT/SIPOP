...
Class specification extends CI_Controller { 
	...
	function index(){
		...
		$component['view_load_form'] = 'specification/specification/form';
	...
	function post_add_edit(){
		...
		$kode_category_spec = isset($_POST['kode_category_spec']) ? $_POST['kode_category_spec'] : '';
	  	$template_detail_id = isset($_POST['template_detail_id']) ? $_POST['template_detail_id'] : 0;
		
		$name_spec = isset($_POST['name_spec']) ? $_POST['name_spec'] : 0;
		$formula_s = isset($_POST['formula_s']) ? $_POST['formula_s'] : 0;
		$formula_xs = isset($_POST['formula_xs']) ? $_POST['formula_xs'] : 0;
		$formula_m = isset($_POST['formula_m']) ? $_POST['formula_m'] : 0;
		$formula_l = isset($_POST['formula_l']) ? $_POST['formula_l'] : 0;
		$formula_xl = isset($_POST['formula_xl']) ? $_POST['formula_xl'] : 0;
		$formula_xxl = isset($_POST['formula_xxl']) ? $_POST['formula_xxl'] : 0;
		
		$formula_pattern_s = isset($_POST['formula_pattern_s']) ? $_POST['formula_pattern_s'] : 0;
		$formula_pattern_xs = isset($_POST['formula_pattern_xs']) ? $_POST['formula_pattern_xs'] : 0;
		$formula_pattern_m = isset($_POST['formula_pattern_m']) ? $_POST['formula_pattern_m'] : 0;
		$formula_pattern_l = isset($_POST['formula_pattern_l']) ? $_POST['formula_pattern_l'] : 0;
		$formula_pattern_xl = isset($_POST['formula_pattern_xl']) ? $_POST['formula_pattern_xl'] : 0;
		$formula_pattern_xxl = isset($_POST['formula_pattern_xxl']) ? $_POST['formula_pattern_xxl'] : 0;
		...
		if(count($_POST) > 0){
		   if($template_detail_id !=''){
			    $this->rpc_service->setSP("dbo.sp_template_spec_detail_edit");
				$this->rpc_service->addField('template_detail_id',$template_detail_id);
				
		     //  $return['valid'] = "true"; //$result['valid'];
		     //  $return['status_code'] ="001"; //$result['no'];
		     //  $return['message'] = $name_spec; //$result['des'];
		   	
		   }else{
			    $this->rpc_service->setSP("dbo.sp_template_spec_detail_add");
		     //  $return['valid'] =  "true"; //$result['valid'];
		     //  $return['status_code'] ="001"; //$result['no'];
		     //  $return['message'] = "template detail kosong"; //$result['des'];
	       }
				...
				if(isset($result['valid'])){
					if($result['valid']){
						if(isset($result['data'])){
							...
							//$return['new_purchase_performa'] = $data_result['new_purchase_performa'];
