								...
								<div class="col-xl-12 mb-10 ml-10">
									<form class="ui grid ecc_form" id="form_<?php echo $methodid ?>" action="javascript:post_<?php echo $methodid ?>()">
										<?php 
											...
											$this->ecc_library->form('hidden','',"form_".$methodid,'app_trans_no_ecc','');										
										?>
										<div class="row">
											<div class="col-xl-4">
												<?php 
												    //$this->ecc_library->form('select','Warehouse',"form_".$methodid,'warehouse_id','','','warehouse');
													//$cek_user_id=[56,12,14,17];
													$cek_user_id=[-99];
													//echo $this->session->userdata('user_id');
													//$tmplate2="ACC/2025-01-01/123";
													if (in_array($this->session->userdata('user_id'),$cek_user_id,true)){
													//if ($this->session->userdata('user_id')== 56){
													  $this->ecc_library->form('text','Good Receive No',"form_".$methodid,'grn_no','','','');
													  $this->ecc_library->form('date','Good Receive Date',"form_".$methodid,'grn_date','',date('Y-m-d'),'');
													}else{
													  $this->ecc_library->form('readonly','Good Receive No',"form_".$methodid,'grn_no','','','');
													  $this->ecc_library->form('readonly','Good Receive Date',"form_".$methodid,'grn_date','',date('Y-m-d'),'');	
													}
																	
													$this->ecc_library->form('select_pop','Template No',"form_".$methodid,'warehouse_id_old','','','warehouse_old');
												?>
											</div>
