<script type="text/javascript">
	var new_subcon_production = 1;
	var subcon_production_plan_id = 0;
	var subcon_production_id = 0;
	var subcon_production_bom_id = 0;
	var subcon_production_quantity = 0;
	var subcon_production_list_id = 0;
	var subcon_production_supply_id = 0;
	...
	function edit_<?php echo $methodid?>(id){
		$.ajax({
			...
			data: {
				...
				,param: 'data_subcon_production'
				,id : id
			},
			dataType: 'json',
			method: 'post',
			success: function(data){
				subcon_production_id = data[0].subcon_production_id;
				new_subcon_production = 0;
				subcon_production_plan_id = data[0].contract_subcon_id;
									
				$('#form_<?php echo $methodid ?>_detail_quantity_subcon').val(0);
				
				$('#form_<?php echo $methodid ?>_subcon_production_id').val(subcon_production_id);
				$('#form_<?php echo $methodid ?>_subcon_production_no').val(data[0].subcon_production_no);
				$('#form_<?php echo $methodid ?>_subcon_production_date').val(data[0].subcon_production_date);
				
				$('#form_<?php echo $methodid ?>_detail_subcon_production_id').val(subcon_production_id);
				$('#form_<?php echo $methodid ?>_detail_subcon_production_detail_id').val('');
				$('#form_<?php echo $methodid ?>_detail_contract_subcon_id').val(subcon_production_plan_id);
				$('#form_<?php echo $methodid ?>_detail_subcon_production_date').val(data[0].subcon_production_date);
				$('#form_<?php echo $methodid ?>_detail_subcon_production_detail_id').val('');
				...
				setTimeout(function(){ 
					change_form_<?php echo $methodid ?>_contract_subcon_id(subcon_production_plan_id);
				}, 500);
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.valid){
						...
						subcon_production_plan_id = $('#form_<?php echo $methodid ?>_contract_subcon_id').val();
						subcon_production_date = $('#form_<?php echo $methodid ?>_subcon_production_date').val();
									
						if(new_subcon_production == 1){
							new_subcon_production = 0;
							...
							$('#form_<?php echo $methodid ?>_detail_subcon_production_id').val(subcon_production_id);
							$('#form_<?php echo $methodid ?>_detail_contract_subcon_id').val(subcon_production_plan_id);
							$('#form_<?php echo $methodid ?>_detail_subcon_production_date').val(subcon_production_date);
	...
	$(function () {
		...
		$("#table_<?php echo $methodid ?>_material").jqGrid({
			...
			postData:{
				'q':'1'
				,'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>'
				, contract_subcon_id : function (){
						return $('#form_<?php echo $methodid ?>_detail_contract_subcon_id').val();	
					}
				
				, subcon_production_date : function (){
						return $('#form_<?php echo $methodid ?>_detail_subcon_production_date').val();	
					}
				, subcon_production_detail_id : function (){
						return $('#form_<?php echo $methodid ?>_detail_subcon_production_detail_id').val();	
					}
			},
			colNames:['item_id', 'Item Code', 'Item Name', 'Qauntity Available', 'Qauntity Material', 'Unit', 'r7'],
			colModel:[
				{
					"name":"r1"
					,"index":"r1"
			...
			loadComplete: function () {
				...
				for (i = 0, count = idsOfSelectedRows_<?php echo $methodid ?>_material.length; i < count; i++) {
	...
	function edit_detail_<?php echo $methodid ?>(id){
		...
		$.ajax({
			...
			data: {
				'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>'
				,param: 'data_subcon_production_detail'
				,id : id
			},
			dataType: 'json',
			method: 'post',
			success: function(data){
				...
				$('#form_<?php echo $methodid ?>_detail_subcon_production_detail_id').val(data[0].subcon_production_detail_id);
			...
			swal({
				...
				if (result.value) {
					...
					$.ajax({
						...
						,subcon_production_detail_id:id},
						dataType: 'json',
						method: 'post',
						success: function(data){
							page_loading("hide");
	...
	var click_transfer_<?php echo $methodid ?> = function (id, isSelected) {
		...
		$('#form_<?php echo $methodid ?>_supply_subcon_production_supply_id').val('');
		...
		if (!isSelected) {
			$('#form_<?php echo $methodid ?>_supply_subcon_production_list_id').val('');
		} else {
			var row = jQuery("#table_<?php echo $methodid ?>_supply").jqGrid('getRowData',id);
			subcon_production_list_id = parseInt(unwrap_cell_value(row.r1).replace(/,/g, ''));
			$('#form_<?php echo $methodid ?>_supply_subcon_production_list_id').val(subcon_production_list_id);
		}
	...
	var click_item_<?php echo $methodid ?> = function (id, isSelected) {	
		if (!isSelected) {
			...
			subcon_production_supply_id = '';
			
			var row_supply = $("#table_<?php echo $methodid ?>_list_transfer").jqGrid('getRowData',id);
			if(unwrap_cell_value(row_supply.r1) == id){
				subcon_production_supply_id = parseInt(unwrap_cell_value(row_supply.r13).replace(/,/g, ''));
			...
			$('#form_<?php echo $methodid ?>_supply_subcon_production_supply_id').val(subcon_production_supply_id);
	...
	var click_supply_<?php echo $methodid ?> = function (id, isSelected) {
		if (!isSelected) {	
			...
			receive_date = unwrap_cell_value(row.r3);
			receive_no = unwrap_cell_value(row.r4);
			subcon_production_supply_id = parseInt(unwrap_cell_value(row.r13).replace(/,/g, ''));
			quantity_supply = parseFloat(unwrap_cell_value(row.r7).replace(/,/g, ''));
	...
	function supply_<?php echo $methodid?>(id){
		$.ajax({
			...
			success: function(data){
				...
				$('#form_<?php echo $methodid ?>_supply_subcon_production_id').val(subcon_production_id);
				$('#form_<?php echo $methodid ?>_supply_subcon_production_list_id').val('');
				$('#form_<?php echo $methodid ?>_supply_subcon_production_no').val(data[0].subcon_production_no);
				$('#form_<?php echo $methodid ?>_supply_subcon_production_date').val(data[0].subcon_production_date);
			...
			swal({
				...
				if (result.value) {
					...
					$.ajax({
						...
						data: {'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>',subcon_production_id:subcon_production_id},
						dataType: 'json',
						method: 'post',
						success: function(data){
							page_loading("hide");
