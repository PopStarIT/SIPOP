...
function nav_button_<?php echo $function ?>_awal(){
		...
		if (id) { 
            ...
            inputOptions: {
               ...
               var data_send={
                  ...
                  ,work_order_transfer_id:row.r1
                  ,format:format
               }; 
      
               $.ajax({
                  type: "POST",
                  //url:baseurl + '<?php echo $class_uri ?>/print_transfer',
				  url:baseurl + '<?php echo $class_uri ?>/print_transfer',
                  data: data_send,
                  dataType : 'json',
                  complete: function(){
	...
	function nav_button_<?php echo $function ?>(){
     var id = jQuery("#table_<?php echo $methodid ?>").jqGrid('getGridParam','selrow');
		if (id) { 
			...
			//alert(row.r8);
			if(row.r8 == 'Pending'){
				show_error("show",'Error','Status Pending');
			}else{
		       ...
		       swal({
                    ...
                    inputOptions: {
                      ...
                      var data_send={
                       ...
                       window.open(baseurl+'<?php echo $class_uri ?>/print_transfer?'+'work_order_transfer_id=' +row.r1+ '&format='+format, '_BLANK');
