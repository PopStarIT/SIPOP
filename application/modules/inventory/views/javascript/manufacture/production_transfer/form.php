<script type="text/javascript">
	...
	//$.fn.modal.Constructor.prototype.enforceFocus = function() {}; //agar input text di swal alert dapat di input tidak disable
	...
	function delete_detail_supply_<?php echo $methodid ?>(id){
		page_loading("show", '<?php echo $page_title ?> Detail_scan', 'Please Wait...');
			
		 $.ajax({
			url: baseurl + '<?php echo $class_uri ?>/delete_supply_sipop',
			data: {
				'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>',
				fabric_transfer_ecc_supply_id: id
	           },
			dataType: 'json',
			method: 'post',
			success: function(data) {
				page_loading("hide");
				if (data.valid) {
				   show_success("show",'delete Supply',data.message);	
				   
				   $("#table_<?php echo $methodid ?>_list_barcode").trigger('reloadGrid');
	               $("#table_<?php echo $methodid ?>_list_barcode").setGridWidth($('.grid_container_<?php echo $methodid; ?>_list_barcode').width() - 20,true).trigger('resize');
				}else {
					show_error("show",'Error',data.message);
				}
			}
	...
	function modal_fabric_transfer_<?php echo $methodid ?>(){
		...
		var id = jQuery("#table_<?php echo $methodid ?>_supply").jqGrid('getGridParam','selrow');
		if (id) { 
			var row = jQuery("#table_<?php echo $methodid ?>_supply").jqGrid('getRowData',id); 
			//alert (row.r1);
			...
			$('#form_<?php echo $methodid ?>_transfer_barcode_work_order_request_list_id').val('');
	...
	//	if (request_list_id === null || request_list_id === '')  { 
	//		show_error("show",'Error','Please select row');
	//		//$('#modal_transfer_barcode').modal('hide');
	//		
	//		setTimeout(function(){ 
	//		$('.tab_scrollbar').getNiceScroll().resize(); 
	//	    }, 100);
  ...
  $('#form_<?php echo $methodid ?>_add_transfer_barcode').keydown(function(e){
	...
	function scan_barcode_<?php echo $methodid ?>(code_barcode) {
		...
		let work_order_request_list_id = $('#form_<?php echo $methodid ?>_transfer_barcode_work_order_request_list_id').val();
		  $.ajax({
			...
			method: 'post',
			success: function(data) {
				fabric_transfer_id: data.fabric_transfer_id;
			   // console.log(data.fabric_transfer_id);
		     ...
		     page_loading("hide");
			    if (data.bc_no == null ){
					...
					html='<label for="nama" style="display: block; text-align: left;">Dokumen : '+ custom_name+'</label><label for="nama" style="display: block; text-align: left;">Nopen : '+ bc_no +'  Date : ' +bc_date+' </label><label for="nama" style="display: block; text-align: left;">Qty available : '+ data.quantity_available+'</label><label for="code" style="display: block; text-align: left;">Item Code : '+ code+ '</label><label for="qty_request" style="display: block; text-align: left;">Qty Request : '+ request+ '</label>';
				}
				
				//var wrapper = document.createElement('div');
               //  wrapper.innerHTML = '<div>Available Stock</div>' +				 
               //                      '<input type="text" id="qty_supply" class="form-control" value="'+data.quantity+'" style="text-align:center;">';
			 
			   if (data.valid) {
				  const fabric_transfer_id = $('#form_<?php echo $methodid ?>_transfer_barcode_fabric_transfer_id').val();
				...
				//  html:html+'<input type="text" id="qty_supply" class="swal2-input" name="qty_supply" value='+data.quantity+'>',
				 html:html+'<input type="text" id="qty_supply" class="swal2-input" name="qty_supply" value='+data.quantity_available+'>',
				  ...
				  focusConfirm: false, // Agar fokus ke input pertama
				   preConfirm: () => {
                     // console.log("Cek ID:", typeof fabric_transfer_id !== 'undefined' ? fabric_transfer_id : "Variabel Gaib");
					  ...
					  var data_send={
							 ...
							 ,work_order_request_list_id: work_order_request_list_id
							 ,keterangan : 1
						};
					  ...
					  $.ajax({
                             ...
                             success: function(msg){
							    ...
							    if (msg.valid) {
								   ...
								   $('#form_<?php echo $methodid ?>_add_transfer_barcode').val('');
	...
	// const beepSound = new Audio("data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YU1vT19ocm9tYSAAAAAAAAAAAAAAAGhwcm9tYSAAAAAAAAAAAAAAAGhwcm9tYSAAAAAAAAAAAAAAAGhwcm9tYSAAAAAAAAAAAAAAAGhwcm9tYSAAAAAAAAAAAAAAAGhwcm9tYSAAAAAAAAAAAAAAA=");
	...
	$('#modal_transfer_barcode').on('hidden.bs.modal', function () {
		 // alert('yytyr');
		 //form_117_transfer_barcode_work_order_request_list_id
		  $('#form_<?php echo $methodid ?>_transfer_barcode_work_order_request_list_id').val('');
		// var inp = $('#form_<?php echo $methodid ?>_transfer_barcode_work_order_request_list_id');
		//  $(this).find('#form_<?php echo $methodid ?>_transfer_barcode_work_order_request_list_id').val('');
		 // var asa = $('#form_<?php echo $methodid ?>_transfer_barcode_work_order_request_list_id').val('');
		 // Kosongkan nilai
         // inp.val('');
		 //console.log("Modal ditutup. Data input '" + inp.val() + "'");
