	...
	function nav_button_<?php echo $function ?>(){
		...
		if (id) { 
			...
			swal({
				...
				if (result.value) {
					page_loading("show",'Send To Ceisa <?php echo $page_title ?>','Please Wait...');
					$.ajax({
						url: baseurl+'<?php echo $class_uri ?>/send_to_ceisa',								
						...
						success: function(data){
							...
							check_submit = 0;
							if(data.valid){
								show_success("show",'Send To Ceisa <?php echo $page_title ?>',data.message);			
