<script type="text/javascript">
	var new_stock_opname = 1;
	var stock_opname_id = 0;
	
	var stock_opname_quantity = 0;
	...
	function edit_<?php echo $methodid?>(id){
		...
		$.ajax({
			...
			success: function(data){
				...
				new_stock_opname = 0;
				stock_opname_id = data[0].stock_opname_id;
								
				$('#form_<?php echo $methodid ?>_stock_opname_id').val(data[0].stock_opname_id);
				$('#form_<?php echo $methodid ?>_stock_opname_no').val(data[0].stock_opname_no);
				$('#form_<?php echo $methodid ?>_stock_opname_date').val(data[0].stock_opname_date);
				$('#form_<?php echo $methodid ?>_stock_opname_memo').val(data[0].stock_opname_memo);
				
				$('#form_<?php echo $methodid ?>_detail_stock_opname_id').val(data[0].stock_opname_id);
				$('#form_<?php echo $methodid ?>_detail_stock_opname_date').val(data[0].stock_opname_date);
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.valid){
						...
						stock_opname_plan_id = $('#form_<?php echo $methodid ?>_work_order_plan_id').val();
						stock_opname_work_process_id = $('#form_<?php echo $methodid ?>_work_process_id').val();
						
						if(new_stock_opname == 1){
							new_stock_opname = 0;
							$("#tab_<?php echo $methodid; ?>_detail").attr("data-toggle","tab");
							$("#tab_<?php echo $methodid; ?>_detail").removeClass( "tab_disabled");
							...
							$('#form_<?php echo $methodid ?>_stock_opname_id').val(stock_opname_id);
							$('#form_<?php echo $methodid ?>_detail_stock_opname_id').val(stock_opname_id);
							$('#form_<?php echo $methodid ?>_detail_work_order_plan_id').val(stock_opname_plan_id);
							$('#form_<?php echo $methodid ?>_detail_work_process_id').val(stock_opname_work_process_id);
	...
	$(function () {
		...
		$("#table_<?php echo $methodid ?>_detail").jqGrid({
			...
			postData:{
					'q':'1'
					,'<?php echo $this->security->get_csrf_token_name() ?>': '<?php echo $this->security->get_csrf_hash() ?>'
					, stock_opname_date : function (){
						return $('#form_<?php echo $methodid ?>_detail_stock_opname_date').val();	
					}
					, stock_opname_id : function (){
						return $('#form_<?php echo $methodid ?>_detail_stock_opname_id').val();	
					}
	...
	function add_<?php echo $methodid ?>(){
		...
		if(check_submit == 0) {
			...
			if (myArray.length == 0 ){
				...
				var data = {
					'<?php echo $this->security->get_csrf_token_name() ?>':'<?php echo $this->security->get_csrf_hash() ?>',                             
					stock_opname_list : myArray,
					stock_opname_id : $('#form_<?php echo $methodid ?>_detail_stock_opname_id').val()
				}
