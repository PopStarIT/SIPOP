...
Class Custom_export extends CI_Controller { 
	...
	function custom_export_transfer_item_table() {
		...
		//$get_field['r4']['formatter'] = 'format_row_supply';
		...
		//$get_field['act']['search'] = false;
		//$get_field['act']['sortable'] = false;
		//$get_field['act']['formatter'] = 'format_row_supply';
		//$get_field['act']['width'] = 300;
	...
	//var_dump('df'.$bc_date );die();
		...
		$jml_report = isset($_POST['mat_rpt_mutasi']) ? $_POST['mat_rpt_mutasi'] : 0;
		//$receive_date = isset($_POST['receive_date']) ? $_POST['receive_date'] : 0;
		
		$total_report = $jml_report - number_format($quantity_supply,10);
		//var_dump($total_report.' total '. $jml_report.' DAN '.$quantity_supply );die();
	  	if ($total_report > 0){
			
		 if(count($_POST) > 0){
			 if($bc_date <= $receive_date){
			      $return['valid'] = false;
				  $return['status_code'] ='0001';
				  $return['message'] = 'Failed, Bc_date < Receive date ';
				//var_dump(' benar '.$receive_date);die();
			  } elseif($stock_move_id == null ){
				$return['valid'] = false;
				$return['status_code'] ='0001';
				$return['message'] = 'Failed, Select Available Stock';
			 }else{
			...
			if(isset($result)){
				if(isset($result['valid'])){
					if($result['valid']){
						if(isset($result['data'])){
							   // logika untuk function di SIPOP
							   //jika data sudah ok maka cek quantity di report mutasi apakah minus atau tidak
							   //jika minus maka data yang sudah di supply akan di update kembali
			...
			$return['valid'] = false;
			$return['message'] = "Failed, Data Report Sudah Minus";
		}			
