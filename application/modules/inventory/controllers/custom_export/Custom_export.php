...
Class Custom_export extends CI_Controller { 
	...
	function approve(){
		...
		if(count($_POST) > 0){
			...
			if($bc_out_header_id){
				$this->rpc_service->setSP("dbo.sp_custom_export_approve");
	...
	function cancel_approve(){
		...
		if(count($_POST) > 0){
			...
			if($bc_out_header_id){
				$this->rpc_service->setSP("dbo.sp_custom_export_cancel_approve");
				$this->rpc_service->addField('bc_out_header_id',$bc_out_header_id);
			}
	...
	function delete(){
		...
		if(count($_POST) > 0){
			...
			if($bc_out_header_id){
				$this->rpc_service->setSP("dbo.sp_custom_export_delete");
				$this->rpc_service->addField('bc_out_header_id',$bc_out_header_id);
			}
	...
	function post_add_edit(){
		...
		$tanggal_surat_jalan = isset($_POST['tanggal_surat_jalan']) ? $_POST['tanggal_surat_jalan'] : '';
		...
		if(count($_POST) > 0){
			if($bc_out_header_id){
				$this->rpc_service->setSP("dbo.sp_custom_export_ceisa40_edit");
				$this->rpc_service->addField('bc_out_header_id',$bc_out_header_id);
			} else {
				switch($bc_out_type_id){
					case '1':
						$this->rpc_service->setSP("dbo.sp_custom_export_ceisa40_add");
					break;
					
					case '2':
						$this->rpc_service->setSP("dbo.sp_custom_export_ceisa40_add_from_performa");
					...
					case '3':
						$this->rpc_service->setSP("dbo.sp_custom_export_add_from_sales");
					...
					case '4':
						$this->rpc_service->setSP("dbo.sp_custom_export_ceisa40_add_from_contract_subcon");
			...
			$this->rpc_service->addField('bc_out_type_id',$bc_out_type_id);
			...
			$this->rpc_service->addField('tanggal_surat_jalan',$tanggal_surat_jalan);
			...
			$this->rpc_service->addField('muat_kppbc_id',$muat_kppbc_id);			
			$this->rpc_service->addField('kppbc_id',$kppbc_id);			
			$this->rpc_service->addField('jenis_ekspor_id',$jenis_ekspor_id);			
			$this->rpc_service->addField('kategori_ekspor_id',$kategori_ekspor_id);			
			$this->rpc_service->addField('cara_perdagangan_id',$cara_perdagangan_id);
			...
			$this->rpc_service->addField('tanggal_perkiraan_ekspor',$tanggal_perkiraan_ekspor);
			...
			$this->rpc_service->addField('muat2_port_id',$muat2_port_id);
			$this->rpc_service->addField('bongkar_port_id',$bongkar_port_id);
			$this->rpc_service->addField('tujuan_port_id',$tujuan_port_id);
			$this->rpc_service->addField('tujuan_country_id',$tujuan_country_id);
			
			$this->rpc_service->addField('jenis_tpb_id',$jenis_tpb_id);
			$this->rpc_service->addField('tujuan_jenis_tpb_id',$tujuan_jenis_tpb_id);
			...
			$this->rpc_service->addField('kode_kantor_periksa',$kode_kantor_periksa);			
			$this->rpc_service->addField('kode_lokasi_periksa',$kode_lokasi_periksa);			
			$this->rpc_service->addField('kode_',$kode_jenis_nilai);			
			...
			$this->rpc_service->addField('tanggal_periksa',$tanggal_periksa);			
			$this->rpc_service->addField('tgl_ttd',$tgl_ttd);			
			...
			$this->rpc_service->addField('kode_pembayar',$kode_pembayar);		
				...
				if(isset($result['valid'])){
					if($result['valid']){
						if(isset($result['data'])){
							...
							//======== Untuk update app_trans_SIPOP============
							if($custom_type_id == 6){
							  $app_trans_id = 39 ; 
							}else if($custom_type_id == 7){
			                  $app_trans_id = 514;
		                    } else if($custom_type_id == 8){
							  $app_trans_id = 515;
					        } else if($custom_type_id == 9){
			                  $app_trans_id = 516;
					        } else if($custom_type_id == 10){
							  $app_trans_id = 517;
		                    } else if($custom_type_id == 11){
							  $app_trans_id = 518;
					        } else if($custom_type_id == 12){
							  $app_trans_id = 519;
				            } else {
							  $app_trans_id = 39;
					        } 
							...
							if(!$bc_out_header_id){
								 $data_no_surat_jalan = explode("/" , $no_surat_jalan);
			                     $app_trans_no=$data_no_surat_jalan[0];
			                        if ($custom_type_id == 10 ){ 
			                        	$no_surat_jalan2=explode("-" , $no_surat);
			                        	$app_trans_no=$no_surat_jalan2[0];
			                        }
								 $data_app_trans =array(
	                               'app_trans_last_date'=>date('Y-m-d'),
								   'app_trans_no' =>$app_trans_no
	                             );
								 
								 $where_cek = array();
				                 $where_cek['app_trans_id'] = $app_trans_id;
		                         $order_cek = null;
								 $data_table_cek = $this->main->getData_pop('dbo.view_app_trans', null, $where_cek, null, $order_cek, 1);
								 $no_trans = $data_table_cek[0]['app_trans_no'];
								 
				                  if ($app_trans_no > $no_trans) {
								  	    $where=array('app_trans_id'=>$app_trans_id);
						                $this->main->update_pop('dbo.dt_app_trans',$data_app_trans,$where);
							            $return['app_trans_no']=$app_trans_no;
								  }
							}
	...
	function post_add_edit_detail(){
		...
		if(count($_POST) > 0){
			if($bc_out_barang_id){
				$this->rpc_service->setSP("dbo.sp_custom_export_detail_ceisa40_edit");
				$this->rpc_service->addField('bc_out_barang_id',$bc_out_barang_id);
			} else {
				$this->rpc_service->setSP("dbo.sp_custom_export_detail_ceisa40_add");
			}
			...
			$this->rpc_service->addField('daerah_kode',$daerah_kode);
			...
			$this->rpc_service->addField('fob',$fob);
			$this->rpc_service->addField('kode_kondisi_barang',$kode_kondisi_barang);
	...
	function post_add_edit_document(){
		...
		if(count($_POST) > 0){
			if($id_dokumen){
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_document_edit");
				$this->rpc_service->addField('id_dokumen',$id_dokumen);
			} else {
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_document_add");
				$this->rpc_service->addField('pl',$pl);
			}
	...
	function post_add_edit_jaminan(){
		...
		if(count($_POST) > 0){
			if($idJaminan){
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_export_jaminan_edit");
				$this->rpc_service->addField('idJaminan',$idJaminan);
			} else {
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_export_jaminan_add");
			}
	...
	function post_add_edit_kontainer(){
		...
		if(count($_POST) > 0){
			if($id_kontainer){
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_kontainer_edit");
				$this->rpc_service->addField('id_kontainer',$id_kontainer);
			} else {
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_kontainer_add");
			}
	...
	function post_add_edit_pkb(){
		...
		if(count($_POST) > 0){
			if($pkb_id){
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_pkb_edit");
				$this->rpc_service->addField('pkb_id',$pkb_id);
			} else {
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_pkb_add");
			}
			
			$this->rpc_service->addField('pkb_id',$pkb_id);
			$this->rpc_service->addField('bc_out_header_id',$bc_out_header_id);
			$this->rpc_service->addField('jenis_barang_id',$jenis_barang_id);
			$this->rpc_service->addField('jenis_gudang_id',$jenis_gudang_id);
			$this->rpc_service->addField('nama_pic',$nama_pic);
			$this->rpc_service->addField('alamat',$alamat);
			$this->rpc_service->addField('nomor_tlp_pic',$nomor_tlp_pic);
			$this->rpc_service->addField('lokasi_siap_periksa',$lokasi_siap_periksa);
			$this->rpc_service->addField('cara_stuffing_id',$cara_stuffing_id);
			$this->rpc_service->addField('jenis_partof_id',$jenis_partof_id);
			$this->rpc_service->addField('tanggal_pkb',$tanggal_pkb);
			$this->rpc_service->addField('waktu_siap_periksa',$waktu_siap_periksa);
			$this->rpc_service->addField('jumlah_kontainer_20',$jumlah_kontainer_20);
			$this->rpc_service->addField('jumlah_kontainer_40',$jumlah_kontainer_40);
	...
	function delete_detail(){
		...
		if(count($_POST) > 0){
			...
			if($bc_out_barang_id){
				$this->rpc_service->setSP("dbo.sp_custom_export_detail_delete");
				$this->rpc_service->addField('bc_out_barang_id',$bc_out_barang_id);
			}
	...
	function delete_document(){
		...
		if(count($_POST) > 0){
			...
			if($id_dokumen){
				$this->rpc_service->setSP("dbo.sp_custom_ceisa_document_delete");
				$this->rpc_service->addField('id_dokumen',$id_dokumen);
			}
	...
	function delete_kemasan(){
		...
		if(count($_POST) > 0){
			...
			if($idJaminan){
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_export_jaminan_delete");
				$this->rpc_service->addField('idJaminan',$idJaminan);
			}
	...
	function delete_kontainer(){
		...
		if(count($_POST) > 0){
			...
			if($id_kontainer){
				$this->rpc_service->setSP("dbo.sp_custom_ceisa_kontainer_delete");
				$this->rpc_service->addField('id_kontainer',$id_kontainer);
			}
	...
	function delete_pkb(){
		...
		if(count($_POST) > 0){
			...
			if($pkb_id){
				$this->rpc_service->setSP("dbo.sp_ceisa_custom_pkb_delete");
				$this->rpc_service->addField('pkb_id',$pkb_id);
			}
	...
	function cetak_dokumen(){
		...
		$q = $this->db->query('
			select DISTINCT ON (bc_out_header_id)
				bc_out_header_id, 
				"nomorAju", 
				"noDaftar", 
				tanggaldaftar, 
				...
				"npwpPengusaha",
				CASE 
					WHEN LENGTH("npwpPengusaha") > 15 THEN '. "'NITKU'" .'
					ELSE NULL
				END AS "textPengusaha",
				CASE 
					WHEN LENGTH("npwpPengusaha") > 15 THEN concat('. "': '" .',"npwpPengusaha",'. "'000000'" .')
					ELSE NULL
				END AS "nitkuPengusaha",
				"nibPengusaha", 
				"nomorIjinPengusaha", 
				...
				"npwpPembeli",
				CASE 
					WHEN LENGTH("npwpPembeli") > 15 THEN '. "'NITKU'" .'
					ELSE NULL
				END AS "textPembeli",
				CASE 
					WHEN LENGTH("npwpPembeli") > 15 THEN concat('. "': '" .',"npwpPembeli",'. "'000000'" .')
					ELSE NULL
				END AS "nitkuPembeli",
				"nibPembeli", 
				"nomorIjinPembeli", 
				...
				"npwpPemilik",
				CASE 
					WHEN LENGTH("npwpPemilik") > 15 THEN '. "'NITKU'" .'
					ELSE NULL
				END AS "textPemilik",
				CASE 
					WHEN LENGTH("npwpPemilik") > 15 THEN concat('. "': '" .',"npwpPemilik",'. "'000000'" .')
					ELSE NULL
				END AS "nitkuPemilik",
				"nibPemilik", 
				"nomorIjinPemilik", 
				...
				valuta,
				CASE
					WHEN "freight" < 1 THEN concat('. "'0'" .', TO_CHAR("freight", '. "'FM99,999,999,999.00'" .'))
					ELSE TO_CHAR("freight", '. "'FM99,999,999,999.00'" .')
					END AS "freight", 
				CASE
					WHEN asuransi < 1 THEN concat('. "'0'" .', TO_CHAR(asuransi, '. "'FM99,999,999,999.00'" .'))
					ELSE TO_CHAR(asuransi, '. "'FM99,999,999,999.00'" .')
					END AS asuransi,
				CASE
			...
			from dbo.view_custom_export_draft
			where bc_out_header_id =  ' . $bc_out_header_id . '
			ORDER BY bc_out_header_id DESC
		');
		...
		foreach($q->result() as $r){
			...
			$textPengusaha = $r-> textPengusaha;
			$nitkuPengusaha = $r-> nitkuPengusaha;
			$textPemilik = $r-> textPemilik;
			$nitkuPemilik = $r-> nitkuPemilik;
			$textPembeli = $r-> textPembeli;
			$nitkuPembeli = $r-> nitkuPembeli;
		...
		$data['textPengusaha'] = $textPengusaha;
		$data['nitkuPengusaha'] = $nitkuPengusaha;
		$data['textPemilik'] = $textPemilik;
		$data['nitkuPemilik'] = $nitkuPemilik;
		$data['textPembeli'] = $textPembeli;
		$data['nitkuPembeli'] = $nitkuPembeli;
		$data['wajib_bayar'] = $wajib_bayar;
